...
export default function GrainRateCalculator({
  commodities,
  ports,
  conditions,
  setConditions,
  ...
  const { countries, portsByCountry } = React.useMemo(() => {
  ...
  const [estDestCountry, setEstDestCountry] = useState("UAE");
  ...
  const [blendRice3Name, setBlendRice3Name] = useState(
    () => savedState.blendRice3Name || "",
  );
  const [blendRice3ExMill, setBlendRice3ExMill] = useState(
    () => savedState.blendRice3ExMill || "0.00",
  );
  const [blendRice3Pct, setBlendRice3Pct] = useState(
    () => savedState.blendRice3Pct || "0",
  );
  ...
  // -----------------------------------------------------------------
  const isBlended = ((industry as any) === "grain" || (industry as any) === "rice_merchant") && commodity === "BLENDED (MIX) RICE";
  const effP1 = (!blendRice1Name || (parseFloat(String(blendRice1ExMill)) || 0) <= 0) ? 0 : (parseFloat(String(blendRice1Pct)) || 0);
  const effP2 = (!blendRice2Name || (parseFloat(String(blendRice2ExMill)) || 0) <= 0) ? 0 : (parseFloat(String(blendRice2Pct)) || 0);
  const effP3 = (!blendRice3Name || (parseFloat(String(blendRice3ExMill)) || 0) <= 0) ? 0 : (parseFloat(String(blendRice3Pct)) || 0);
  
  const hasErr1 = (parseFloat(String(blendRice1Pct || "0")) > 0) && (!blendRice1Name || (parseFloat(String(blendRice1ExMill)) || 0) <= 0);
  const hasErr2 = (parseFloat(String(blendRice2Pct || "0")) > 0) && (!blendRice2Name || (parseFloat(String(blendRice2ExMill)) || 0) <= 0);
  const hasErr3 = (parseFloat(String(blendRice3Pct || "0")) > 0) && (!blendRice3Name || (parseFloat(String(blendRice3ExMill)) || 0) <= 0);

  const computedBlendExMill = isBlended
    ? ((parseFloat(String(blendRice1ExMill)) || 0) * effP1 +
        (parseFloat(String(blendRice2ExMill)) || 0) * effP2 +
        (parseFloat(String(blendRice3ExMill)) || 0) * effP3) /
      100
    : parseFloat(exmill) || 0;
  ...
  const isBlendInvalid = isBlended && (effP1 + effP2 + effP3) !== 100;
  ...
  const canOverride = (() => {
    ...
    if (finalLandedUsdUnit <= 0 || isBlendInvalid) return false;
  ...
  const handleSaveToRateList = () => {
    if (isBlended) {
      if ((effP1 + effP2 + effP3) !== 100) {
        alert("Total valid Blend Ratio must equal 100%. Ensure named varieties with prices > 0 are selected for your ratios.");
        return;
      }
    }
      ...
      item = {
        ...
        ...(isBlended
          ? {
              ...
              blendRice3Name,
              blendRice3Pct: parseFloat(blendRice3Pct) || 0,
              blendRice3ExMill: parseFloat(blendRice3ExMill) || 0,
              blendCookingRemarks,
    ...
    if (backfillItem && backfillItem.data) {
      ...
      if (
        ...
        if (d.blendRice1Name || d.blendRice2Name) {
          ...
          if (d.blendRice3Name) setBlendRice3Name(d.blendRice3Name);
          if (d.blendRice3Pct) setBlendRice3Pct(String(d.blendRice3Pct));
          if (d.blendRice3ExMill)
            setBlendRice3ExMill(String(d.blendRice3ExMill));
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="card bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-950 border border-indigo-900 text-white p-5 rounded-2xl shadow-xl flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="bg-slate-950/40 p-4 border border-white/5 rounded-xl text-center relative group">
                {isBlendInvalid ? (
                  <div className="text-rose-400 font-bold text-sm min-h-[4rem] flex flex-col justify-center">
                    <div>⚠️ Invalid Blend Ratio</div>
                    <div className="text-[10px] mt-1 text-rose-300">Total must be exactly 100%. Currently: {(effP1 + effP2 + effP3)}%</div>
                  </div>
                ) : (
                  <>
          ...
          <div className="card bg-[#f8fafc] p-4.5 rounded-xl border border-gray-200 shadow-xs space-y-3">
            ...
            {!isBlendInvalid && <div className="bg-[#0f4c81] text-white p-2.5 rounded-lg text-center font-extrabold uppercase text-[11px] tracking-wider mt-2.5 shadow-sm">

...
interface RateListBoardProps {
  rateList: RateRow[];
  setRateList: React.Dispatch<React.SetStateAction<RateRow[]>>;
...
export default function RateListBoard({
  ...
  // Filter list rows
  const industryFilteredRates = rateList.filter(
    (r) => (r.industry || "grain") === industry,
  );

  const visibleRates = industryFilteredRates.filter((row) => {
  ...
  const activePorts = Array.from(
    new Set(industryFilteredRates.map((r) => r.dest)),
  ).sort();
  ...
  const handleExportCSV = (exportSelected = false) => {
    const listToExport = exportSelected
      ? industryFilteredRates.filter((r) => selectedIds.has(r.id))
      : industryFilteredRates;
    ...
    link.setAttribute(
      "download",
      `rice_export_rates_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handleClearAllBoard = () => {
    if (industryFilteredRates.length === 0) return;
    setShowClearConfirm(true);
  };

  const confirmClearAllBoard = () => {
    setRateList((prev) =>
      prev.filter((r) => (r.industry || "grain") !== industry),
    );
    setSelectedIds(new Set());
  ...
  const confirmDeleteRow = () => {
    ...
    setRateList((prev) => prev.filter((r) => r.id !== deleteConfirmId));
    setSelectedIds((prev) => {
      const next = new Set(prev);
  ...
  const handleOpenAddModal = (rowToEdit?: RateRow) => {
    if (isTrialMode) {
      if (rowToEdit) {
        alert(
          "Licence required! Editing saved rates is disabled in Trial Mode.",
        );
        return;
      }
      if (industryFilteredRates.length >= 5) {
        alert("Licence required! Maximum 5 rates can be tested in Trial Mode.");
        return;
      }
    }
  ...
  const handleSmartImport = (items: any[]) => {
    let newItems = [];
    const now = new Date();
    ...
    items.forEach((item, idx) => {
      newItems.push({
        id: now.getTime() + idx,
        dest: item.dest || "UNKNOWN",
        commodity: item.commodity || "UNKNOWN",
        brand: item.brand || "NOT-DEFINED",
        packed: item.packed || "Standard",
        size: item.size || "Standard",
        master: item.master || "Standard",
        crop: "NEW",
        year: now.getFullYear().toString(),
        rate: 0,
        condition: "FCL",
        paymentTerms: "T/T",
        numFCL: item.numFCL ? parseInt(item.numFCL) || 1 : 1,
        weightPerContainerKg: 0,
        totalWeightKg: 0,
        date: now.toISOString().split('T')[0],
      });
    });

    setRateList(prev => [...prev, ...newItems]);
    
    // Show toast for success
    setSavedToast({ show: true, msg: `${newItems.length} items imported successfully!`, success: true });
  ...
  const handleSaveModalValue = () => {
    ...
    if (!mDest || !mComm || isNaN(rateVal) || rateVal <= 0) {
      alert(
        "Please fill out all required parameters. Rate must be greater than 0",
      );
      return;
    }
    ...
    const compiledRow: RateRow = {
      ...
      weightPerContainerKg: editId
        ? rateList.find((r) => r.id === editId)?.weightPerContainerKg || 26000
        : 26000,
      totalWeightKg: editId
        ? rateList.find((r) => r.id === editId)?.totalWeightKg || 26000
        : 26000,
      industry: editId
        ? rateList.find((r) => r.id === editId)?.industry
        : industry,
      date: editId
        ? rateList.find((r) => r.id === editId)?.date ||
    ...
    if (editId) {
      setRateList((prev) =>
        prev.map((r) => (r.id === editId ? compiledRow : r)),
      );
    } else {
      setRateList((prev) => [compiledRow, ...prev]);
    }
  ...
  return (
    <div className="space-y-4" id="board-rates-view">
      ...
      {savedToast && savedToast.show && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-lg shadow-xl animate-fade-in-up border ${savedToast.success ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
          {savedToast.success ? <CheckCircle2 className="w-5 h-5 text-emerald-600" /> : <AlertCircle className="w-5 h-5 text-red-600" />}
          <span className="text-sm font-bold">{savedToast.msg}</span>
        </div>
      )}
      <div className="page-header">
        <div className="breadcrumb">📈 Pricing Terminal</div>
        <h2 className="text-xl font-extrabold tracking-tight">
          Export Rate List Board
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          Review, select and group stored prices. Use checkboxes on various
          grades to forge multi-port printable catalogs.
        </p>
      </div>
      ...
      {activePorts.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto py-1 scrollbar-none">
          ...
          >
            All Port Zones ({industryFilteredRates.length})
          </button>
          {activePorts.map((portCode) => {
            const count = industryFilteredRates.filter(
              (row) => row.dest === portCode,
            ).length;
            return (
              <button
                key={portCode}
            ...
            <thead>
              <tr className="bg-gray-50 uppercase tracking-widest font-extrabold text-[9px] text-gray-500 border-b border-gray-200">
                <th className="p-3" style={{ width: "40px" }}>
                  ...
                  >
                    {selectedIds.size === visibleRates.length &&
                ...
                <th className="p-3">
                  {(industry as any) === "tiles" || (industry as any) === "solar_panel" ? "Product Style" : "Commodity Spec"}
                </th>
                <th className="p-3">Brand</th>
                <th className="p-3">
                  {(industry as any) === "tiles"
                    ? "Container/Pallet Packing"
                    : "Packaging Type"}
                </th>
                <th className="p-3">
                  {(industry as any) === "tiles" || (industry as any) === "solar_panel" ? "Box / Panel Size" : "Weight Size"}
                </th>
                <th className="p-3 text-center">
                  {(industry as any) === "tiles" ? "Palletized" : "Double Master"}
                </th>
                <th className="p-3">
                  {(industry as any) === "tiles" ? "Grade Quality" : "Crop Quality"}
                </th>
                <th className="p-3 text-right">
                  Landed USD/{(industry as any) === "tiles" ? "SQM" : (industry as any) === "solar_panel" ? "UNIT" : "MT"}
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {visibleRates.length === 0 ? (
                <tr>
                  ...
                  >
                    {industryFilteredRates.length === 0
                ...
                visibleRates.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className="p-3 font-extrabold text-blue-900 bg-amber-50/20 max-w-[150px] truncate uppercase font-sans select-all"
                        title={row.buyer || "Not Specified"}
                      >
                        {row.buyer || (
                          <span className="text-gray-300 italic font-normal text-[10px]">
                            not specified
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3 font-semibold text-gray-800 break-words max-w-[200px] leading-tight">
                        ...
                        {row.bExMill ? (
                          <div className="text-[10px] text-gray-500 font-mono mt-0.5">
                            Supplier Base: ₹{row.bExMill.toFixed(2)} 
                            {(industry === "oil" || industry === "agri_multi" || row.size?.includes(" L") || row.size?.includes(" BOTTLE")) ? "/ L" : "/ KG"}
                            {(industry === "oil" || industry === "agri_multi") && row.size && (row.size.includes(" L") || row.size.includes("BOTTLE") || row.size.includes("JERRY CAN") || row.size.includes("TIN CAN")) ? (
                              <span className="ml-2 text-indigo-500 font-bold">
                                (Raw/Bottle: ₹{(row.bExMill * (parseFloat(row.size.replace(/[^0-9.]/g, '')) || 1)).toFixed(2)})
                              </span>
                            ) : null}
                          </div>
                        ) : null}
                        {(() => {
                          ...
                          if (processedAvailable >= requiredTons) {
                            return (
                              <span
                                className="inline-flex items-center gap-1 mt-1 bg-emerald-50 text-emerald-700 border border-emerald-100 px-1.5 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider"
                            ...
                            return (
                              <span
                                className="inline-flex items-center gap-1 mt-1 bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider"
                                title={`Short processed stock by ${shortfall.toFixed(1)} MT. Silo has ${paddyAvailable.toFixed(1)} MT Paddy. Click to order milling.`}
                              >
                            ...
                            return (
                              <span
                                className="inline-flex items-center gap-1 mt-1 bg-rose-50 text-rose-700 border border-rose-100 px-1.5 py-0.5 rounded text-[8.5px] font-black uppercase tracking-wider animate-pulse font-bold"
                      ...
                      <td className="p-3 font-mono text-gray-600 uppercase">
                        {row.brand}
                      </td>
                      ...
                      <td className="p-3 text-gray-500 font-semibold text-[11px]">
                        {(industry as any) === "tiles" ? (
                          <>
                            {row.crop === "NEW" ? "PREMIUM" : "STANDARD"} Grade
                          </>
                        ) : (
                          <>
                      ...
                      <td
                        className="p-3 text-gray-500 max-w-[150px] truncate"
                        title={row.paymentTerms}
                      >
                        {row.paymentTerms || "-"}
                      </td>
                      <td className="p-3" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-zoomIn flex flex-col max-h-[90vh]">
            <div className="bg-gradient-to-r from-blue-700 to-blue-800 text-white px-4 py-3.5 flex items-center justify-between">
              <h3 className="font-extrabold text-sm">
                {editId ? "Modify Rate Row" : "Add Manual Export price"}
              </h3>
              <button
                onClick={() => setShowAddModal(false)}
                className="text-white/80 p-0.5 hover:text-white rounded"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="field">
                  ...
                  >
                    ...
                    <option value="DOUBLE BROWN JUTE BAGS">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="field">
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-gray-300 rounded p-1.5 text-xs font-semibold outline-none focus:border-blue-500"
                  >
      ...
      {/* Clear All Board Confirm Modal */}
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-red-50 text-red-700">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                <AlertCircle className="w-4 h-4" /> Clear All Rates?
              </h3>
            </div>
            <div className="p-6 text-sm text-gray-700">
              Are you sure you want to clear EVERY rate from the board? This
              cannot be undone.
            </div>
            <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex justify-end gap-2 shrink-0">
              <button
      ...
      {deleteConfirmId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-red-50 text-red-700">
              <h3 className="font-extrabold text-sm flex items-center gap-2">
                <AlertCircle className="w-4 h-4" /> Delete Rate Log?
              </h3>
            </div>
            <div className="p-6 text-sm text-gray-700">
              Are you sure you want to delete this rate log from the board?
            </div>
            <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex justify-end gap-2 shrink-0">
              <button
                type="button"

...
export default function OilRateCalculator({
  commodities,
  ports,
  conditions,
  setConditions,
  ...
  useEffect(() => {
    fetchOnlineExchangeRate(false, baseCurrency);
  }, [baseCurrency]);
  ...
  React.useEffect(() => {
    if (
      industry === "oil" &&
      ((bagtype || "").includes("BOTTLE") || (bagtype || "").includes("CAN") || (bagtype || "").includes("CANS"))
    ) {
      setOilPricingMode("PIECE");
    } else if (industry === "oil" && ((bagtype || "") === "FLEXITANKS" || (bagtype || "") === "STEEL DRUMS")) {
  ...
  if (
    ...
    unitLabel = "MT";
    if (industry === "oil" && oilPricingMode === "PIECE") {
      unitLabel = "L";
    }
    costUnitLabel = "L";
    ...
    // Box packing calculations for oil (masterbag cost used as box packing derivative)
    let masterBagCostKg = 0;
    if (oilPricingMode === "PIECE") {
      if (oilUseBox === "YES") {
    ...
    // Convert to USD per Metric Ton or Bottle/Piece
    ...
    if (targetUsdNum > 0 && exrateNum > 0) {
      ...
      reverseHtmlOutput = `Required Ex-Mill ${baseCurrency}: $₹ ${Math.max(rExmill, 0).toFixed(2)} / KG`;
    }
    ...
    if (targetUsdNum > 0 && exrateNum > 0) {
      ...
      const otherCosts =
        ...
        freightSqmInr;
      const rExmill = targetBaseCost - otherCosts;
      reverseHtmlOutput = `Required Tile Ex-Mill ${baseCurrency}: $₹ ${Math.max(rExmill, 0).toFixed(2)} / SQM`;
    }
    ...
    if (targetUsdNum > 0 && exrateNum > 0) {
      ...
      const otherCosts =
        ...
        freightUnitInr;
      const rExmill = targetBaseCost - otherCosts;
      reverseHtmlOutput = `Required Cargo Ex-Mill ${baseCurrency}: $₹ ${Math.max(rExmill, 0).toFixed(4)} / UNIT`;
    }
  ...
  const handleSaveToRateList = () => {
    ...
    if (
      ...
      item = {
        ...
        oilPricingMode: (industry as any) === "oil" ? oilPricingMode : undefined,
        oilUseBox: (industry as any) === "oil" ? oilUseBox : undefined,
        oilPiecesPerBox: (industry as any) === "oil" && oilUseBox === "YES" ? (parseFloat(oilPiecesPerBox) || 1) : undefined,
        totalBottlesPerFcl: (industry as any) === "oil"
          ? Math.round((parseFloat(fclWeight) || 26000) / (parseFloat(bagsize) || 1))
          : undefined,
        totalBoxesPerFcl: (industry as any) === "oil" && oilUseBox === "YES" && (parseFloat(oilPiecesPerBox) > 0)
          ? Math.round(((parseFloat(fclWeight) || 26000) / (parseFloat(bagsize) || 1)) / parseFloat(oilPiecesPerBox))
          : undefined,
        ratePerKl: (industry as any) === "oil"
          ...
          {((industry as any) === "grain" ||
            ...
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                ...
                <div className="field">
                  <label>{(industry as any) === "oil" || bagtype.includes("BOTTLE") ? "Bottle Volume (L)" : indConfig.sizeLabel}</label>
                  <select
                    value={bagsize}
                ...
                <div className="field">
                  <label>{(industry as any) === "oil" || bagtype.includes("BOTTLE") ? `Cost ($₹ / Bottle)` : indConfig.costLabel}</label>
                  <input
                    type="number"
                    step="0.01"
                    value={bagcost}
          ...
          {(industry as any) === "tiles" && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="border-t border-gray-100 pt-3 mt-2 grid grid-cols-1 md:grid-cols-5 gap-3">
                ...
                {/* Auto Calculated Pack Count */}
                <div className="field">
                  <label className="truncate text-slate-500">Total Packages (Est.)</label>
                  <div className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono font-black text-center text-slate-700 outline-none select-none">
                    {parseFloat(fclWeight) > 0 && parseFloat(bagsize) > 0 
                      ? Math.floor(parseFloat(fclWeight) / parseFloat(bagsize)).toLocaleString() 
                      : 0} 
                    {" "} {(industry as any) === "oil" || bagtype.includes("BOTTLE") ? "BOTTLES" : "BAGS/PCS"}
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="field">
                  ...
                  >
                    {(
                      ...
                      <option key={sObj.size} value={String(sObj.size)}>
                        {sObj.size} {((industry as any) === "oil" || bagtype.includes("BOTTLE") ? "L / KG" : "KG")} Packing
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="field">
                ...
                <div className="flex gap-1 relative">
                  <select 
                    ...
                    className="bg-slate-100 border border-gray-300 rounded-xl px-1 py-2 text-[10px] font-bold text-slate-700 outline-none w-16 cursor-pointer"
                  >

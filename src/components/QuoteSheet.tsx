...
export default function QuoteSheet({
  ...
  const industryIntroFormat =
    ((industry as any) === "grain" || (industry as any) === "rice_merchant")
      ...
      : (industry as any) === "spices"
        ...
        : ((industry as any) === "chemicals" || (industry as any) === "polymer" || (industry as any) === "cardboard_carton")
                ...
                : (industry as any) === "metal"
                  ...
                  : (industry as any) === "sugar"
                    ...
                    : (industry as any) === "nuts"
                      ...
                      : (industry as any) === "cardboard_carton"
                        ? "premium Cardboard, Kraft Paper, and Packaging solutions"
                        : (industry as any) === "polymer" || (industry as any) === "cardboard_carton"
                        ? "high-quality Polymers & Plastics, manufactured to demanding industrial standards"
                        : (industry as any) === "solar_panel"
                        ? "high-efficiency Solar Panels and Renewable Energy Equipment, manufactured to precision international standards"
  ...
  const [tableColumns, setTableColumns] = useState([
    ...
    {
      id: "commodity",
      label: (industry as any) === "tiles" || (industry as any) === "solar_panel" ? "Product Style" : "Commodity",
      visible: true,
    },
    ...
    {
      id: "rate",
      label: `Rate / ${(industry as any) === "tiles" ? "SQM" : (industry as any) === "solar_panel" ? "Unit/Watt" : "MT"}`,
      visible: true,
    },
  ]);
  ...
  const handleSaveQuote = () => {
    if (isFreeTier) {
      ...
      const activeNonCreditQuotes = savedQuotes.filter(q => !q.isPaidCredit);
      
      if (activeNonCreditQuotes.length >= 2 && !hasSingleShipmentCredits) {
        showToast("Free tier allows a maximum of 2 saved quotations. Please delete one of your saved folders to continue, or upgrade to a premium plan.", true);
        return;
      }
      
      const currentCalcs = licenceMetadata?.freeTierCalcsThisMonth || 0;
      if (currentCalcs >= 4 && !hasSingleShipmentCredits) {
        const getNextResetDateStr = () => {
          const now = new Date();
          const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1);
          return nextMonth.toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
        };
        showToast(`You've used all 4 free calculations this month. Upgrade to continue, or your limit resets on ${getNextResetDateStr()}.`, true);
        return;
      }
    }
    if (!isSavingAllowed) {
    ...
    <div className="space-y-6" id="quote-sheet-page">
      ...
      <div className="page-header flex flex-col md:flex-row md:items-center justify-between gap-3 select-none no-print">
        ...
        <div className="flex gap-2.5">
          ...
          <button
            ...
            className="px-3.5 py-1.5 border border-emerald-200 text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded-lg text-xs font-extrabold flex items-center gap-1.5 shadow-xs transition"
          >
            <FileCheck className="w-4 h-4" /> Export PDF
          </button>
      ...
      >
        ...
        >
                      ...
                      case "totalWeight":
                        return (
                          <>
                            ...
                            {(industry as any) === "tiles" ? "BOXES" : (industry as any) === "solar_panel" ? "UNITS" : "KG"}
                          </>
                        );
                      case "rate": {
          ...
          {isFreeTier && (
            <div className="border-t border-dashed border-gray-100 mt-4 pt-3 text-center font-mono text-[10px] text-gray-400 tracking-widest uppercase">
              ExportProDoc
            </div>
          )}
        </div>

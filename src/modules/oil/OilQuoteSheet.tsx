...
export default function OilQuoteSheet({
  selectedRateIds,
  rateList,
  savedQuotes,
  setSavedQuotes,
                  ...
                  const renderCellContent = (colId: string) => {
                    switch (colId) {
                      ...
                      case "rate": {
                        ...
                        const isPiece = row.oilPricingMode === "PIECE";
                        const bottlesPerFcl = row.totalBottlesPerFcl || Math.round((row.weightPerContainerKg || 26000) / bottleSizeNum);
                        const weightPerFclKl = (row.weightPerContainerKg || 26000) / 1000;
                        const finalRatePerKl = row.ratePerKl || (isPiece ? row.rate * (1000 / bottleSizeNum) : row.rate);
                        const fclCost = isPiece
                          ? row.rate * bottlesPerFcl
                          : finalRatePerKl * weightPerFclKl;
                        ...
                        return (
                          <>
                            ...
                            {isOilItem && (
                              <div className="text-[0.8em] font-medium font-mono text-left text-indigo-700 mt-1 space-y-0.5">
                                ...
                                <div className="font-black text-emerald-700 text-[0.9em]">
                                  FCL Cost: ${fclCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </div>
                              </div>
                            )}

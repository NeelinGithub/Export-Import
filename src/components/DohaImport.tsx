...
import { DohaImportData, DohaExpenseRow, SavedQuote, ExpenseItem } from '../types';
import { 
...
interface DohaImportProps {
  ...
  appFclWeight?: number;
  bagPrices?: { [key: string]: Array<{ size: number; price: number; id: string }> };
}
...
export default function DohaImport({
  ...
  // Profit Simulation States
  const [sellPriceQar, setSellPriceQar] = useState<number>(0);
  const [bagsSoldSim, setBagsSoldSim] = useState<number>(0);
  ...
  const totalLocalWeight = fcl * containerWeightKg;
  let dynamicInlandRsPerKg = 0;
  if (totalLocalWeight > 0) {
    const lineItemsSum = lineItems.reduce((acc, item) => acc + ((item.apply === 'fcl' ? item.qty * fcl : item.qty) * item.rate * (1 + item.gst / 100)), 0);
    const cfsItemsSum = cfsItems.reduce((acc, item) => acc + ((item.apply === 'fcl' ? item.qty * fcl : item.qty) * item.rate * (1 + item.gst / 100)), 0);
    dynamicInlandRsPerKg = (lineItemsSum + cfsItemsSum) / totalLocalWeight;
  }

  // Auto-sync inlandRsPerKg based on selected FCL in Doha Import
  useEffect(() => {
    if (dynamicInlandRsPerKg > 0) {
      setDohaImport(prev => {
        const val = parseFloat(dynamicInlandRsPerKg.toFixed(4));
        if (prev.inlandRsPerKg !== val) {
          return { ...prev, inlandRsPerKg: val };
        }
        return prev;
      });
    }
  }, [dynamicInlandRsPerKg, setDohaImport]);

  const bagType = dohaImport.bagType || Object.keys(bagPrices)[0] || 'pp';

  // Find bag price
  let derivedPackRsPerKg = dohaImport.packingRsPerKg || 0;
  if (bagPrices[bagType]) {
    const bagRecord = bagPrices[bagType].find(b => b.size === bagSizeKg);
    if (bagRecord && bagRecord.price > 0 && bagSizeKg > 0) {
      derivedPackRsPerKg = bagRecord.price / bagSizeKg;
    }
  }

  // Auto-sync packingRsPerKg based on selected bagType/size
  useEffect(() => {
    if (derivedPackRsPerKg > 0) {
      setDohaImport(prev => {
        const val = parseFloat(derivedPackRsPerKg.toFixed(4));
        if (prev.packingRsPerKg !== val) {
          return { ...prev, packingRsPerKg: val };
        }
        return prev;
      });
    }
  }, [derivedPackRsPerKg, setDohaImport]);

  const calcMode = dohaImport.calcMode || 'direct_cif';
  const exMillRs = dohaImport.exMillRsPerKg || 0;
  const packRs = dohaImport.packingRsPerKg || 0;
  const cfsTransRs = dohaImport.inlandRsPerKg || 0;
  const usdInrRate = dohaImport.usdInrRate || 84.5;
  const freightUsdFcl = dohaImport.freightUsdPerFcl || 0;

  const fobRsKg = exMillRs + packRs + cfsTransRs;
  const fobUsdMt = usdInrRate > 0 ? (fobRsKg / usdInrRate) * 1000 : 0;
  const cargoWeightMt = containerWeightKg / 1000;
  const freightUsdMt = cargoWeightMt > 0 ? (freightUsdFcl / cargoWeightMt) : 0;
  const derivedCifUsdMt = fobUsdMt + freightUsdMt;

  const activeCifUsd = calcMode === 'exmill_build' ? derivedCifUsdMt : cifRateUsd;

  // CIF calculations
  const cifMtQar = activeCifUsd * exchangeQarUsd;
  const cifBagUsd = (activeCifUsd / 1000) * bagSizeKg;
  ...
  const handleSelectQuoteAndItem = (quote: SavedQuote, item: any) => {
    ...
    setDohaImport((prev) => ({
      ...
      bagType: item.packed || Object.keys(bagPrices)[0] || 'pp',
      cargoName: item.commodity,
      selectedQuoteRef: quote.ref,
      dutyBase: parseFloat(computedCifBagQar.toFixed(2)),
      calcMode: 'exmill_build',
      exMillRsPerKg: item.bExMill || 0,
      packingRsPerKg: item.bPackaging || 0,
      usdInrRate: item.exrate || 84.5,
      freightUsdPerFcl: item.bFreight && item.exrate ? (item.bFreight * payloadWeight) / item.exrate : 0,
    }));
    ...
    <div className="space-y-6" id="doha-customs-landing-page">
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-200 rounded-xl p-4 flex flex-wrap gap-5 items-center justify-between shadow-xs select-none">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 flex-1 min-w-[280px]">
          ...
          <div>
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-0.5">Consignment Volume</span>
        ...
        <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-4 lg:col-span-2">
          ...
          <div className="flex justify-between items-center border-b pb-2">
            ...
            <button
              ...
              className="bg-teal-50 hover:bg-teal-100 text-teal-700 hover:text-teal-800 border border-teal-200 rounded-lg px-2.5 py-1 text-xs font-black flex items-center gap-1 transition-all cursor-pointer shadow-xs"
            ...
            <div className="space-y-3">
              <div className="field">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 text-xs text-teal-950 font-bold outline-none focus:border-teal-500 focus:bg-white"
                  placeholder="e.g. SONA MASOORI SELLA RICE"
                />
              </div>
              
              <div className="border border-gray-200 rounded-lg p-2.5 bg-white shadow-xs">
                <div className="flex items-center gap-2 mb-2">
                  <button 
                    onClick={() => handleMetaChange('calcMode', 'direct_cif')}
                    className={`flex-1 text-[9px] font-bold uppercase py-1 rounded transition ${calcMode === 'direct_cif' ? 'bg-teal-100 text-teal-800' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                  >
                    Direct CIF ($)
                  </button>
                  <button 
                    onClick={() => handleMetaChange('calcMode', 'exmill_build')}
                    className={`flex-1 text-[9px] font-bold uppercase py-1 rounded transition ${calcMode === 'exmill_build' ? 'bg-teal-100 text-teal-800' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                  >
                    Ex-Mill Builder (₹)
                  </button>
                </div>
                ...
                {calcMode === 'direct_cif' ? (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="field">
                      ...
                      <div className="flex items-center bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 focus-within:border-teal-500 focus-within:bg-white transition">
                  ...
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
                      <div className="field col-span-1">
                        <label className="text-[8px] uppercase font-bold text-gray-400 mb-0.5 block">Ex-Mill (₹/kg)</label>
                        <input type="number" step="0.01" value={exMillRs} onChange={(e) => handleMetaChange('exMillRsPerKg', parseFloat(e.target.value) || 0)} className="w-full text-xs font-mono font-bold bg-gray-50 p-1.5 border border-gray-200 rounded outline-none focus:border-teal-500" placeholder="0.00"/>
                      </div>
                      <div className="field col-span-1">
                        <label className="text-[8px] uppercase font-bold text-gray-400 mb-0.5 block">Pack (₹/kg)</label>
                        <input type="number" step="0.01" value={packRs} onChange={(e) => handleMetaChange('packingRsPerKg', parseFloat(e.target.value) || 0)} className="w-full text-xs font-mono font-bold bg-gray-50 p-1.5 border border-gray-200 rounded outline-none focus:border-teal-500" placeholder="0.00"/>
                        <span className="text-[8px] text-teal-600 font-bold mt-0.5 block">₹{(packRs * bagSizeKg).toFixed(1)} / bag</span>
                      </div>
                      <div className="field col-span-1">
                        <label className="text-[8px] uppercase font-bold text-gray-400 mb-0.5 block flex justify-between">
                          <span>CFS+Trans</span>
                          <button onClick={() => handleMetaChange('inlandRsPerKg', parseFloat(dynamicInlandRsPerKg.toFixed(4)))} className="text-[8px] text-teal-600 font-black hover:text-teal-700 bg-teal-50 px-0.5 rounded" title="Auto Calculate from FCL selection">A: {dynamicInlandRsPerKg.toFixed(2)}</button>
                        </label>
                        <input type="number" step="0.01" value={cfsTransRs} onChange={(e) => handleMetaChange('inlandRsPerKg', parseFloat(e.target.value) || 0)} className="w-full text-xs font-mono font-bold bg-gray-50 p-1.5 border border-gray-200 rounded outline-none focus:border-teal-500" placeholder="0.00"/>
                      </div>
                      <div className="field col-span-1">
                        <label className="text-[8px] uppercase font-bold text-gray-400 mb-0.5 block">USD-INR Rate</label>
                        <input type="number" step="0.01" value={usdInrRate} onChange={(e) => handleMetaChange('usdInrRate', parseFloat(e.target.value) || 0)} className="w-full text-xs font-mono font-bold bg-gray-50 p-1.5 border border-gray-200 rounded outline-none focus:border-teal-500" />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      <div className="field">
                        <label className="text-[8px] uppercase font-bold text-gray-400 mb-0.5 block">Sea Freight ($/FCL)</label>
                        <input type="number" step="1" value={freightUsdFcl} onChange={(e) => handleMetaChange('freightUsdPerFcl', parseFloat(e.target.value) || 0)} className="w-full text-xs font-mono font-bold bg-gray-50 p-1.5 border border-gray-200 rounded outline-none focus:border-teal-500" />
                      </div>
                      <div className="flex items-end justify-end">
                        <div className="text-right">
                          <span className="text-[8px] text-gray-400 uppercase font-bold block leading-none">Derived CIF / MT</span>
                          <span className="text-sm font-mono font-black text-indigo-700 block leading-none mt-0.5">${derivedCifUsdMt.toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="field">
                    <label className="text-[10px] uppercase tracking-wider font-bold text-gray-400 block mb-1">Bag Type</label>
                    <div className="flex items-center bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 focus-within:border-teal-500 focus-within:bg-white transition">
                      <select 
                        value={bagType}
                        onChange={(e) => handleMetaChange('bagType', e.target.value)}
                        className="w-full bg-transparent outline-none text-xs font-mono font-bold text-gray-800"
                      >
                        {Object.keys(bagPrices).map(type => (
                  ...
                  <div className="field">
                    <label className="text-[10px] uppercase tracking-wider font-bold text-gray-400 block mb-1">Size (KG)</label>
                    <div className="flex items-center bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 focus-within:border-teal-500 focus-within:bg-white transition">
                      <select 
                        value={String(bagSizeKg)}
                        onChange={(e) => handleMetaChange('bagSizeKg', parseInt(e.target.value) || 35)}
                        className="w-full bg-transparent outline-none text-xs font-mono font-bold text-gray-800"
                      >
                        {(bagPrices[bagType] || []).map((sObj) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="field">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 text-xs font-mono font-bold outline-none focus:border-teal-500 focus:bg-white text-center"
                  />
                </div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded px-2.5 py-1.5 text-xs font-mono font-bold text-center outline-none ${
                      bagsManual 
                        ...
                        : 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed font-semibold'
                    }`}
            ...
            <div className="field">
              ...
              <div className="flex items-center bg-gray-50 border border-gray-300 rounded px-2.5 py-1.5 focus-within:border-teal-500 focus-within:bg-white transition">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-0 outline-none text-xs font-mono font-bold text-right text-gray-800"
                />
              </div>
              <span className="text-[9px] text-gray-400 block mt-0.5">Customs valuation benchmark ad-valorem base</span>
            </div>
          </div>
        ...
        <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-4">
          <div className="flex justify-between items-center border-b pb-2">
            ...
            <button
              ...
              className="text-[9px] font-bold text-teal-600 hover:text-teal-800 disabled:text-gray-400 flex items-center gap-1 bg-teal-50/10 hover:bg-teal-50/20 rounded px-1.5 py-0.5 transition cursor-pointer"
          ...
          <div className="field">
            ...
            <div className="flex items-center bg-white border border-gray-300 rounded px-3 py-1.5 focus-within:border-teal-500 transition">
              ...
              />
              <span className="text-[10px] text-gray-400 ml-2 font-bold shrink-0">QAR = $1 USD</span>
            </div>
      ...
      <div className="card bg-gradient-to-br from-teal-900 to-slate-900 text-white p-5 rounded-xl border border-teal-950/40 shadow-md space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 divide-y md:divide-y-0 md:divide-x divide-teal-800/50">
          ...
          <div className="space-y-1 md:pl-4 pt-3 md:pt-0">
            <span className="text-[9px] font-black text-teal-400 uppercase tracking-wider block">4. Final Doha Landed Cost</span>
            <div className="space-y-0.5">
              <span className="text-2xl font-black font-mono block text-teal-300 select-all">
                QAR {(cifBagQar + costPerBagQar).toFixed(3)} <span className="text-xs font-black text-white bg-teal-800 px-1.5 py-0.5 rounded ml-1">/ Bag</span>
              </span>
        ...
        <div className="bg-teal-950/60 p-3 rounded-lg border border-teal-800 text-[11px] text-teal-100 flex flex-wrap gap-x-6 gap-y-1 justify-between items-center font-mono select-none">
          <div>
      ...
      <div className="card bg-white p-4 rounded-xl border border-indigo-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-100 pb-3">
          <div className="space-y-0.5">
            <h3 className="text-sm font-black text-indigo-900 uppercase flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4 text-indigo-600" /> Margin & Sales Profit Simulation
            </h3>
            <p className="text-xs text-gray-500">Calculate net sales margin over total final landed cost in QAR.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-3">
            <div className="field">
              <label className="text-[10px] uppercase font-bold text-gray-500 block mb-1">Selling Price per Bag (QAR)</label>
              <div className="flex items-center bg-gray-50 border border-indigo-200 rounded px-2.5 py-1.5 focus-within:border-indigo-500 focus-within:bg-white transition">
                <span className="text-[10px] text-gray-400 font-bold mr-1">QAR</span>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  value={sellPriceQar || ""}
                  onChange={(e) => setSellPriceQar(parseFloat(e.target.value) || 0)}
                  className="w-full bg-transparent outline-none text-sm font-mono font-black text-indigo-800"
                  placeholder={(cifBagQar + costPerBagQar).toFixed(2)}
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="field">
              <label className="text-[10px] uppercase font-bold text-gray-500 block mb-1">Number of Bags Sold</label>
              <div className="flex items-center bg-gray-50 border border-indigo-200 rounded px-2.5 py-1.5 focus-within:border-indigo-500 focus-within:bg-white transition">
                <input
                  type="number"
                  min="1"
                  step="1"
                  value={bagsSoldSim || ""}
                  onChange={(e) => setBagsSoldSim(parseInt(e.target.value) || 0)}
                  className="w-full bg-transparent outline-none text-sm font-mono font-black text-indigo-800"
                  placeholder={totalBags.toString()}
                />
                <span className="text-[10px] text-gray-400 font-bold ml-1">Bags</span>
              </div>
            </div>
          </div>

          <div className="border border-indigo-100 bg-indigo-50/50 rounded-lg p-3 flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest block mb-0.5">Estimated Profit</span>
            {(() => {
              const currentLandedBag = cifBagQar + costPerBagQar;
              const inputSellPrice = sellPriceQar > 0 ? sellPriceQar : currentLandedBag;
              const actualBagsSold = bagsSoldSim > 0 ? bagsSoldSim : totalBags;
              const profitPerBag = inputSellPrice - currentLandedBag;
              const totalProfit = profitPerBag * actualBagsSold;
              const isPositive = totalProfit >= 0;
              return (
                <>
                  <div className={`text-xl font-black font-mono ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {isPositive ? '+' : ''}{totalProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} QAR
                  </div>
                  <span className={`text-[10px] font-bold ${isPositive ? 'text-emerald-500' : 'text-rose-500'}`}>
                    ({isPositive ? '+' : ''}{profitPerBag.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} QAR / bag)
                  </span>
                </>
              );
            })()}
      ...
      <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-4">
        ...
        <div className="bg-gray-50 p-3 border border-gray-200 rounded-lg space-y-3">
          <span className="text-xs font-black text-gray-700 block">+ Append Custom Clearance Fee Row</span>
          <div className="flex flex-col sm:flex-row gap-2.5 items-end">
            ...
            <div className="w-full sm:w-36">
              ...
              <select
                ...
                className="w-full bg-white border border-gray-300 rounded px-2 py-1.5 text-xs outline-none focus:border-teal-500 font-semibold"
              >
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-gray-200">
              {dohaImport.rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-teal-50/10 transition duration-150">
                    <td className="p-2">
                      <input
                        type="text"
                        value={row.name}
                    ...
                    <td className="p-2 text-center">
                      <button
                        ...
                        className="text-gray-400 hover:text-red-600 p-1 rounded transition close-btn"
      ...
      {showQuoteSelector && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-subtle flex items-center justify-center p-4 z-55 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-3 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row gap-2.5 items-center">
              <div className="relative flex-1 w-full">
                ...
                <input
                  ...
                  className="w-full bg-white pl-9 pr-3 py-1.5 text-xs text-gray-800 font-semibold border border-gray-300 rounded-lg outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
                />
              </div>
                          ...
                          <div key={idx} className="bg-gray-50 hover:bg-teal-50/40 p-2.5 rounded-lg border border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition">
                            <div className="space-y-0.5">
                              ...
                              <div className="text-[9.5px] font-mono text-teal-800 font-bold mt-0.5">
                                CIF Price: <strong className="text-teal-900">${(item.rate || 0).toLocaleString()} USD/MT</strong> ({(item.weightPerContainerKg || 26050).toLocaleString()} kg / container)
                              </div>
                            </div>
                            <button
                              type="button"
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-100 flex justify-end">
              <button
                type="button"
                onClick={() => setShowQuoteSelector(false)}
                className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-1.5 rounded-lg text-xs font-semibold cursor-pointer"
              >
                Close
              </button>
            </div>

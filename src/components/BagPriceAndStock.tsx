...
export default function BagPriceAndStock({
  ...
  // Merge default categories with any dynamic ones added to bagPrices
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {categories.map((category, idx) => {
            ...
            return (
              <div
                key={category + '-' + idx}
                className="bg-white border border-slate-200 hover:border-indigo-300 hover:shadow-lg transition-all rounded-xl p-4 flex flex-col justify-between relative overflow-hidden group shadow-sm ring-1 ring-black/5"
              >
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-indigo-50 to-transparent -z-10 group-hover:scale-110 transition-transform"></div>
                <div>
                  <h4 className="text-[11px] tracking-wide font-black text-slate-800 uppercase border-b border-slate-100 pb-2 mb-3 flex justify-between items-center relative z-10">
                    <span className="flex items-center gap-1.5"><ShoppingBag className="w-3.5 h-3.5 text-indigo-500" /> {category}</span>

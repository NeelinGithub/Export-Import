...
export default function RateCalculator({
  ...
  if (
    ...
    const totalPackagingKg = bagCostKg + masterBagCostKg;
        ...
        <div className="xl:col-span-2 space-y-4 font-sans">
                ...
                <label className="flex justify-between items-center w-full">
                  ...
                  {setPorts && (
                    <button
                      type="button"
                      onClick={() => {
                        const country = window.prompt("Enter Country for the new port:");
                        if (!country) return;
                        const portName = window.prompt("Enter New Port Name in " + country + ":");
                        if (!portName) return;
                        const finalPort = country.toUpperCase().trim() + " - " + portName.toUpperCase().trim();
                        if (!ports.includes(finalPort)) {
                          console.log("Port saved", finalPort);
                        }
                        setPort(finalPort);
                      }}
                      className="text-[9px] font-black uppercase text-blue-600 hover:text-blue-800 flex items-center gap-0.5 cursor-pointer"
                    >
          ...
          {(industry as any) === "tiles" && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="border-t border-gray-100 pt-3 mt-2 grid grid-cols-1 md:grid-cols-4 gap-3">
                <div className="field">
                  <label>{indConfig.packLabel}</label>
                  <select
                    value={bagtype}
          ...
          {(industry as any) === "generic" && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="border-t border-gray-200 pt-3 mt-2 grid grid-cols-1 md:grid-cols-4 gap-3">
                <div className="field">
                  <label>{indConfig.packLabel}</label>
                  <select
                    value={bagtype}

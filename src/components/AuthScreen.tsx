...
import { isQuotaExceeded } from '../firebase';
import { 
  Lock, Mail, UserPlus, LogIn, ChevronRight, Info, Eye, EyeOff, AlertCircle, Sparkles, Check, Globe
} from 'lucide-react';
        ...
        if (country) {
          if (!isQuotaExceeded()) {
            try {
              ...
              console.warn("Could not save user country due to Firestore quota or error");
            }
          }
        }
    ...
    <div className="flex flex-col min-h-screen bg-slate-900 text-slate-100 justify-between font-sans items-center px-4 py-8" id="rice_export_auth_portal">
      ...
      <div className="max-w-md w-full bg-slate-950/80 border border-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-md space-y-5">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white hover:bg-slate-100 active:bg-slate-200 text-slate-900 font-extrabold text-xs uppercase tracking-wider py-3.5 rounded-xl transition flex items-center justify-center gap-2 shadow-lg cursor-pointer"
        >
        ...
        <div className="grid grid-cols-2 p-1 bg-slate-900 border border-slate-800 rounded-xl">
          <button
            ...
            className={`py-2 text-[10px] font-black uppercase rounded-lg transition-all ${
              !isSignUp ? 'bg-sky-500 text-slate-950 shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            Sign In
          </button>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 focus:border-sky-500 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-slate-500 transition font-medium focus:ring-1 focus:ring-sky-500 outline-none"
              />
            </div>
          </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    required
                    className="w-full bg-slate-900 border border-slate-800 focus:border-sky-500 rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-slate-500 transition font-medium focus:ring-1 focus:ring-sky-500 outline-none appearance-none"
                  >
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••"
                className="w-full bg-slate-900 border border-slate-800 focus:border-sky-500 rounded-xl pl-10 pr-10 py-2.5 text-xs text-white placeholder-slate-500 transition font-medium focus:ring-1 focus:ring-sky-500 outline-none"
              />
              <button
                type="button"

...
import { Ship, Search, Calendar, MapPin, Copy, Clock, Printer, X, Check, Lock, ChevronRight, AlertCircle, FileText, ArrowRight, ExternalLink } from 'lucide-react';
...
export default function VesselScheduleExplorer({
  ...
  const generateSchedulesOnRoute = (pol: string, pod: string, departBase: string, isAlternative: boolean = false, warRisk: boolean = true): SailingVessel[] => {
    ...
    if (cleanPod.includes('jebel ali') || cleanPod.includes('dubai') || cleanPod.includes('uae')) {
      ...
      baseTransit = 4;
      baseFreight = 1250;
    } else if (cleanPod.includes('rotterdam') || cleanPod.includes('hamburg') || cleanPod.includes('europe')) {
      baseTransit = 24;
      baseFreight = 3800;
    } else if (cleanPod.includes('newark') || cleanPod.includes('houston') || cleanPod.includes('america')) {
      baseTransit = 30;
      baseFreight = 4800;
    } else {
      // Deterministic fallback transit days based on pod string length hash
      baseTransit = 6 + (pod.length % 9);
      baseFreight = 1500 + (pod.length * 100);
      if (baseTransit > 9) viaJebelAli = true;
    }

    // Apply War Risk Cape Detour logically
    const isDeepGulfOrRedSea = ['hamad', 'jeddah', 'dammam', 'riyadh', 'shuwaikh', 'shuaiba', 'doha', 'qatar', 'manama'].some(p => cleanPod.includes(p));
    if (isDeepGulfOrRedSea && warRisk) {
      baseTransit += 14;
      baseFreight += 3450; // Detour war premium
    } else if (warRisk && (cleanPod.includes('jebel ali') || cleanPod.includes('sohar') || cleanPod.includes('dubai') || cleanPod.includes('oman'))) {
      baseFreight += 350; // Regular war risk premium without detour
    }
    ...
    const carrierSpecs = [
      ...
      { name: 'NVOCC / Consolidator', color: 'text-violet-500 border-violet-500/30', bg: 'bg-violet-500/5', service: 'NVX', dayOffset: 0, space: 'confirmed' }
    ];
    ...
    const vesselsOnCarrier: Record<string, string[]> = {
      ...
      'NVOCC / Consolidator': ['TBA (NVOCC SLOT)', 'COMMON CARRIER SL', 'CHARTER VESSEL TBA', 'SHARED ALLIANCE VSL']
    };
    ...
    for (let week = 0; week < 3; week++) {
      carrierSpecs.forEach((carrier, cIdx) => {
        ...
        results.push({
          ...
          oceanFreightEst: Math.round((baseFreight + (week * 25) + (cIdx * 35)) * (isAlternative ? 0.90 : 1)) // Option B offers a 10% bulk freight discount
        });
      });
    }
  ...
  const schedules = generateSchedulesOnRoute(origin, destination, selectedDate, activeTab === 'optionB', applyWarRisk);
  ...
  const handleDirectBook = (sched: SailingVessel) => {
    let url = '';
    const carrierName = sched.carrier.toLowerCase();
    
    if (carrierName.includes('msc')) {
      url = 'https://www.msc.com/en/ebusiness/book';
    } else if (carrierName.includes('maersk')) {
      url = 'https://www.maersk.com/booking/';
    } else if (carrierName.includes('cma cgm')) {
      url = 'https://www.cma-cgm.com/ebusiness/booking';
    } else if (carrierName.includes('hapag')) {
      url = 'https://www.hapag-lloyd.com/en/online-business/quotation/quick-quotes.html';
    } else if (carrierName.includes('ocean network') || carrierName.includes('one')) {
      url = 'https://ecomm.one-line.com/';
    } else if (carrierName.includes('milaha')) {
      url = 'https://www.milaha.com';
    } else {
      url = 'https://www.google.com/search?q=' + encodeURIComponent(sched.carrier + ' cargo booking online');
    }
  ...
  const handleApplyVesselToQuote = (sched: SailingVessel) => {
    ...
    setTimeout(() => {
      ...
      // Attempt to close if it is a standalone popup
      if (onClose) onClose();
      try {
        window.close();
      } catch (e) {}
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans" id="vessel-schedule-explorer-view">
      ...
      <header className="bg-slate-950 border-b border-slate-800 px-6 py-4 flex flex-wrap gap-4 items-center justify-between no-print sticky top-0 z-50 animate-in fade-in">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <div className="flex items-center gap-1.5 p-1 bg-slate-950 border border-slate-800 rounded-xl max-w-max">
                <button
                  type="button"
              ...
              {/* War Risk Manual Override Toggle */}
              {destination.toUpperCase().match(/JEDDAH|JEBEL ALI|SOHAR|DAMMAM|RIYADH|SHUWAIKH|SHUAIBA|DOHA|QATAR|MANAMA/) && (
                <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 rounded-xl p-1.5 ml-2">
                  <label className="relative inline-flex items-center cursor-pointer select-none">
                    ...
                    />
                    <div className="w-8 h-4 bg-slate-800 rounded-full peer peer-focus:ring-2 peer-focus:ring-rose-500 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[3px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-2.5 after:w-2.5 after:transition-all peer-checked:bg-rose-600"></div>
                    <span className="ml-[6px] text-[9.5px] font-bold text-rose-400 uppercase tracking-wider tooltip" title="Toggle when Red Sea crisis normalses to drop diversion transit times & premiums">
        ...
        <div className="flex items-center gap-2 no-print">
          <button
            ...
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-xs border border-slate-800 text-slate-300 rounded-xl font-bold flex items-center gap-1.5 transition"
          >
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in duration-350">
          ...
          <div className="lg:col-span-1 bg-slate-900 border border-slate-800/80 rounded-2xl p-5 h-max space-y-5 no-print">
            
            {/* Active RFQ Metadata block */}
            {activeQuote && (
              <div className="bg-blue-950/30 border border-blue-900/40 p-3.5 rounded-xl space-y-2 select-text">
                <span className="text-[9px] font-black tracking-widest text-sky-400 uppercase block">ACTIVE RFQ TARGET</span>
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] flex gap-2 pt-1 border-t border-slate-800 text-slate-400">
                ...
                {activeQuote.vesselFlightNo && (
                  <div className="mt-2.5 pt-2 border-t border-slate-800 text-[10.5px]">
            ...
            <div className="pt-2">
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-[10px] text-slate-400 leading-normal space-y-1">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex gap-4 items-center">
                <div className="px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-center min-w-[90px]">
            ...
            <section className="bg-slate-900 border border-slate-800/80 rounded-2xl p-5 space-y-4 no-print">
              ...
              <div className="relative pt-6 pb-2">
                ...
                <div className="relative flex justify-between">
                  ...
                  {/* Step 2 */}
                  <div className="flex flex-col items-center text-center space-y-1 relative z-10">
                    <span className="w-5 h-5 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-[9px] font-black text-slate-400">2</span>
                    <span className="text-[9.5px] font-black text-slate-400 block uppercase pt-1">Customs Clear</span>
                  ...
                  {/* Step 4 */}
                  <div className="flex flex-col items-center text-center space-y-1 relative z-10">
                    <span className="w-5 h-5 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-[9px] font-black text-slate-400">4</span>
                    <span className="text-[9.5px] font-black text-slate-400 block uppercase pt-1">Port Discharge</span>
              ...
              {filteredSchedules.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {filteredSchedules.map((sched) => (
                    <div 
                      key={sched.id} 
                      className="bg-slate-900 border border-slate-800/70 hover:border-slate-700 p-4 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 transition select-text"
                    >
                      ...
                      <div className="flex flex-row md:flex-col gap-2 shrink-0 justify-end">
                        <button
                          onClick={() => handleDirectBook(sched)}
                          className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-black uppercase rounded-lg shadow-md transition flex items-center justify-center gap-1.5 leading-none w-full cursor-pointer"
                          title="Direct Book on Carrier Portal"
                        >
                          <ExternalLink className="w-3 h-3" /> Direct Book
                        </button>

                        {activeQuote && (
                          <button
                            onClick={() => handleApplyVesselToQuote(sched)}
                            className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-black uppercase rounded-lg shadow-md transition flex items-center justify-center gap-1.5 leading-none w-full cursor-pointer"
                            title="Apply this vessel, voyage, and dates directly to reference quote worksheets"
                          >
                            <Check className="w-3 h-3" /> LINK TO PI
                          </button>
                        )}

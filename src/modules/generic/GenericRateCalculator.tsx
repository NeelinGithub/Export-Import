...
import { ALL_CURRENCIES, getCurrencySymbol } from "../../utils/currency";
...
import { getReadyMadeCountries, getReadyMadePortsForCountry, READY_MADE_PORTS } from "../../utils/portsDatabase";
...
const getSortedCountries = (countryList: string[]) => {
  const frequent = countryList.filter(c => FREQUENT_COUNTRIES.includes(c.toUpperCase()));
  const regular = countryList.filter(c => !FREQUENT_COUNTRIES.includes(c.toUpperCase())).sort();
  return [...frequent, ...regular];
};

const getSortedPorts = (portList: string[]) => {
  const frequent = portList.filter(p => FREQUENT_PORTS.includes(p.toUpperCase()));
  const regular = portList.filter(p => !FREQUENT_PORTS.includes(p.toUpperCase())).sort();
  return [...frequent, ...regular];
};
...
export default function GenericRateCalculator({
  commodities,
  ports,
  conditions,
  setConditions,
  ...
  const [selectedCountry, setSelectedCountry] = useState(() => {
    if (savedState.port && savedState.port.includes(" - ")) {
      return savedState.port.split(" - ")[0];
    }
    return "";
  });
  ...
  const [baseCurrency, setBaseCurrency] = useState(() => savedState.baseCurrency || localStorage.getItem("rems_default_currency") || "INR");
  ...
  const fetchOnlineExchangeRate = async (showNotification = false, targetCurrency = baseCurrency) => {
    ...
    try {
      ...
      if (data && data.rates && data.rates[targetCurrency]) {
        const liveInr = data.rates[targetCurrency];
        const roundedRate = (Math.round(liveInr * 100) / 100).toFixed(2);

        // Convert exmill, transport, cfsCharge and blend formulation exmills if baseCurrency has changed
        if (targetCurrency !== baseCurrency && data.rates[baseCurrency]) {
          const oldRate = data.rates[baseCurrency];
          const factor = liveInr / oldRate;

          const parsedExmill = parseFloat(exmill) || 0;
          if (parsedExmill > 0) {
            setExmill((parsedExmill * factor).toFixed(2));
          }

          const parsedTransport = parseFloat(transport) || 0;
          if (parsedTransport > 0) {
            setTransport((parsedTransport * factor).toFixed(2));
          }

          const parsedCfs = parseFloat(cfsCharge) || 0;
          if (parsedCfs > 0) {
            setCfsCharge((parsedCfs * factor).toFixed(2));
          }

          const parsedBlendRice1ExMill = parseFloat(blendRice1ExMill) || 0;
          if (parsedBlendRice1ExMill > 0) {
            setBlendRice1ExMill((parsedBlendRice1ExMill * factor).toFixed(2));
          }

          const parsedBlendRice2ExMill = parseFloat(blendRice2ExMill) || 0;
          if (parsedBlendRice2ExMill > 0) {
            setBlendRice2ExMill((parsedBlendRice2ExMill * factor).toFixed(2));
          }
        }
        ...
        if (showNotification) {
          const toast = document.getElementById("toast");
          if (toast) {
            toast.textContent = `Exchange rate synced successfully: ${getCurrencySymbol(targetCurrency)} ${roundedRate} / USD`;
  ...
  const { countries, portsByCountry = {} } = useMemo(() => {
    const pbc: Record<string, string[]> = {};
    const cSet = new Set<string>();

    // Start with all ready-made ports around the world from ports database
    for (const [c, pList] of Object.entries(READY_MADE_PORTS)) {
      cSet.add(c);
      pbc[c] = [...pList];
    }

    // Now merge any custom or extra ones from ports prop
    ports.forEach(p => {
      let country = "OTHER";
      let portName = p;
      if (p.includes(" - ")) {
    ...
    return { countries: Array.from(cSet).sort(), portsByCountry: pbc };
  }, [ports]);
  ...
  const [estSourceCountry, setEstSourceCountry] = useState("INDIA");
  const [estSource, setEstSource] = useState("MUNDRA");
  const [estDest, setEstDest] = useState(port || "JEBEL ALI");
  const [estDestCountry, setEstDestCountry] = useState(() => {
    const p = port || "JEBEL ALI";
  ...
  const getFclEstimateRate = (
    ...
    const src = srcPort.includes(" - ") ? srcPort.split(" - ")[1].toUpperCase().trim() : srcPort.toUpperCase().trim();
    const dst = dstPort.includes(" - ") ? dstPort.split(" - ")[1].toUpperCase().trim() : dstPort.toUpperCase().trim();
  ...
  const detectCountryForPort = (portName: string) => {
    if (!portName) return "";
    if (portName.includes(" - ")) return portName.split(" - ")[0];
    
    // Look up in the parsed portsByCountry
    for (const [country, pListRaw] of Object.entries(portsByCountry || {})) {
      const pList = pListRaw as string[];
      if (pList && pList.some(p => p.toUpperCase() === portName.toUpperCase())) {
        return country;
      }
    }
    
    // Fallback well-known mappings
    const upperPort = portName.toUpperCase();
    if (["MUNDRA", "NHAVA SHEVA (JNPT)", "HAZIRA", "KANDLA", "CHENNAI", "KAKINADA", "KOLKATA", "COCHIN", "TUTICORIN"].includes(upperPort)) return "INDIA";
    if (["JEBEL ALI", "SOHAR", "HAMAD", "DAMMAM", "RIYADH", "JEDDAH", "MUSCAT", "MANAMA", "SHUWAIKH", "SHUAIBA", "BANDAR ABBAS", "DOHA"].includes(upperPort)) {
      if (upperPort === "JEBEL ALI") return "UAE";
      if (upperPort === "SOHAR" || upperPort === "MUSCAT") return "OMAN";
      if (upperPort === "HAMAD" || upperPort === "DOHA") return "QATAR";
      if (upperPort === "DAMMAM" || upperPort === "RIYADH" || upperPort === "JEDDAH") return "SAUDI ARABIA";
      if (upperPort === "MANAMA") return "BAHRAIN";
      if (upperPort === "SHUWAIKH" || upperPort === "SHUAIBA") return "KUWAIT";
      return "OTHER";
    }
    if (["ROTTERDAM", "HAMBURG", "LONDON", "GENOA", "ANTWERP", "FELIXSTOWE", "BARCELONA", "VALENCIA", "LE HARVE"].includes(upperPort)) {
      if (upperPort === "ROTTERDAM") return "NETHERLANDS";
      if (upperPort === "HAMBURG") return "GERMANY";
      if (upperPort === "LONDON" || upperPort === "FELIXSTOWE") return "UK";
      if (upperPort === "GENOA" || upperPort === "BARCELONA" || upperPort === "VALENCIA") return "SPAIN";
      return "OTHER";
    }
    if (["NEWARK", "NEW YORK", "HOUSTON", "SAVANNAH", "LOS ANGELES", "SEATTLE"].includes(upperPort)) return "USA";
    if (["VANCOUVER", "TORONTO", "MONTREAL", "HALIFAX"].includes(upperPort)) return "CANADA";
    
    return "OTHER";
  };

  // Sync selectedCountry when port changes in the main calculator
  useEffect(() => {
    if (port && port.includes(" - ")) {
      const parts = port.split(" - ");
      if (parts[0] !== selectedCountry) {
        setSelectedCountry(parts[0]);
      }
    }
  }, [port]);

  // Sync inputs when estimator modal opens
  useEffect(() => {
    if (showFreightEstimator) {
      // Default sourcing country to India, sourcing port to MUNDRA
      setEstSourceCountry("INDIA");
      setEstSource("INDIA - MUNDRA");

      // Default discharge port to primary calculator's port
      if (port) {
        const detectedCountry = detectCountryForPort(port);
        setEstDestCountry(detectedCountry);
        
        const portPart = port.includes(" - ") ? port.split(" - ")[1] : port;
        if (detectedCountry && detectedCountry !== "OTHER") {
          setEstDest(`${detectedCountry} - ${portPart}`);
        } else {
          setEstDest(portPart);
        }
      } else {
        setEstDestCountry("UAE");
        setEstDest("UAE - JEBEL ALI");
      }
    }
  }, [showFreightEstimator]);

  // Sync calculations whenever query params change
  useEffect(() => {
    if (showFreightEstimator) {
  ...
  const [isCustomPortInput, setIsCustomPortInput] = useState(false);
  ...
  const [sizeUnit, setSizeUnit] = useState(() => savedState.sizeUnit || "KG");
  ...
  const [oilPricingMode, setOilPricingMode] = useState<"MT" | "PIECE">("MT");
  const [oilUseBox, setOilUseBox] = useState(() => savedState.oilUseBox || "NO");
  const [oilPiecesPerBox, setOilPiecesPerBox] = useState(() => savedState.oilPiecesPerBox || "4");
  const [oilBoxPrice, setOilBoxPrice] = useState(() => savedState.oilBoxPrice || "45.00");
  const [manualBottles, setManualBottles] = useState(() => savedState.manualBottles || "");
  ...
  React.useEffect(() => {
    if (industry === "agri_multi" && bagtype.includes("BOTTLE")) {
      setOilPricingMode("PIECE");
    } else if (industry === "agri_multi" && (bagtype === "KL STANDARD" || bagtype === "FLEXITANKS")) {
      setOilPricingMode("MT");
    }
  }, [bagtype, industry]);
  ...
  useEffect(() => {
    ...
    let validCategories = config.categories;
    if ((industry as any) === "agri_multi") {
      if (/\bOIL\b/.test(commodity.toUpperCase())) {
        validCategories = ["PET BOTTLES", "GLASS BOTTLES", "TIN CANS", "HDPE JERRY CANS", "FLEXITANKS", "STEEL DRUMS", "IBC TOTES"];
      } else {
        validCategories = config.categories.filter(c => !["PET BOTTLES", "GLASS BOTTLES", "TIN CANS", "HDPE JERRY CANS", "FLEXITANKS", "IBC TOTES", "STEEL DRUMS"].includes(c));
      }
    }

    // Validate if the current bagtype belongs to this industry. If not, fallback to the first active packaging type.
    if (!Array.from(new Set([...validCategories, ...Object.keys(bagPrices || {})])).filter((cat) => !((bagPrices || {})[cat]?.[0]?.id === "DELETED")).includes(bagtype)) {
      const defaultCategory = Array.from(new Set([...validCategories, ...Object.keys(bagPrices || {})])).filter((cat) => !((bagPrices || {})[cat]?.[0]?.id === "DELETED"))[0] || config.categories[0];
  ...
  const handleCommoditySelect = (name: string) => {
    ...
    if ((industry as any) === "agri_multi") {
      if (/\bOIL\b/.test(name.toUpperCase())) {
        setBagtype("PET BOTTLES");
        setBagsize("1");
      } else if (name.toUpperCase().includes("RICE") || name.toUpperCase().includes("SUGAR")) {
        setBagtype("PP BAGS");
        setBagsize("50");
      } else if (name.toUpperCase().includes("WHEAT") || name.toUpperCase().includes("BARLEY") || name.toUpperCase().includes("CORN")) {
        setBagtype("BULK");
        setBagsize("1000");
      } else if (name.toUpperCase().includes("MAIZE") || name.toUpperCase().includes("SOYBEAN")) {
        setBagtype("PP BAGS");
        setBagsize("50");
      }
    }
    ...
    if ((industry as any) === "vegetables_fruits") {
      ...
      setBagcost("18.00");
      setFclWeight("26000");
    } else if ((industry as any) === "agri_multi") {
      if (/\bOIL\b/.test(nameUpper)) {
        setBagtype("PET BOTTLES");
        setBagsize("5");
        setBagcost("45.00");
        setFclWeight("18000");
      } else if (nameUpper.includes("WHEAT") || nameUpper.includes("BARLEY") || nameUpper.includes("CORN") || nameUpper.includes("MAIZE")) {
        // Bulk for wheat, corn, barley. But user says "Maize and Soybeans: Use low-quality bags similar to rice."
        if (nameUpper.includes("MAIZE") || nameUpper.includes("SOYBEAN")) {
            setBagtype("WHITE PP BAGS");
            setBagsize("50");
            setBagcost("16.00");
            setFclWeight("26000");
            ...
            setBagtype("BULK");
            setBagsize("1000"); // arbitrary divisor, cost is forced to 0
            setBagcost("0.00");
            setFclWeight("26000");
        }
      } else {
  ...
  // -----------------------------------------------------------------
  const isBlended = ((industry as any) === "grain" || (industry as any) === "agri_multi") && commodity === "BLENDED (MIX) RICE";
  ...
  const isOilMode = (industry as any) === "oil" || ((industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()));
  ...
  let costUnitLabel = "KG";
    if ((industry as any) === "oil" || ((industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()))) {
      costUnitLabel = oilPricingMode === "PIECE" ? "BOTTLE" : "KL";
    }
  ...
  if (
    ...
    unitLabel = "MT";
    const isOilModeCalculated = (industry as any) === "oil" || ((industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()));
    if (isOilModeCalculated) {
      unitLabel = oilPricingMode === "PIECE" ? "BOTTLE" : "KL";
    }
    costUnitLabel = "KG";
    if (isOilModeCalculated) {
      costUnitLabel = oilPricingMode === "PIECE" ? "BOTTLE" : "KL";
    ...
    let exmillNum = isBlended ? computedBlendExMill : parseFloat(exmill) || 0;
    ...
    let bagCostKg = bagCostNum / bagSizeNum;
    if ((industry as any) === "agri_multi" && (commodity.toUpperCase().includes("WHEAT") || commodity.toUpperCase().includes("CORN") || commodity.toUpperCase().includes("BARLEY") || bagtype.toUpperCase() === "BULK")) {
        bagCostKg = 0; // Supplied in bulk
    }

    // Master bag calculations
    let masterBagCostKg = 0;
    
    if (masterbag === "YES" && !isOilMode) {
    ...
    if (isOilMode && oilUseBox === "YES") {
      const boxPrice = parseFloat(oilBoxPrice) || 0;
      const piecesBox = parseFloat(oilPiecesPerBox) || 1;
      if (bagSizeNum > 0) {
        masterBagCostKg = (boxPrice / piecesBox) / bagSizeNum;
      }
    }
    ...
    // Convert to USD per Metric Ton or Bottle
    let unitMultiplier = 1000;
    if (isOilMode && oilPricingMode === "PIECE") {
      unitMultiplier = parseFloat(bagsize) || 1;
    }
    const baseUsdMt = exrateNum > 0 ? (totalInrKg / exrateNum) * unitMultiplier : 0;
    finalLandedUsdUnit = baseUsdMt + commissionNum;

    // Scale breakdown elements for KL
    if (costUnitLabel === "KL") {
      bExMill = bExMill * 1000;
      bPackaging = bPackaging * 1000;
      bTransport = bTransport * 1000;
      bCfsPort = bCfsPort * 1000;
      bFreight = bFreight * 1000;
      bDuty = bDuty * 1000;
      bTotalCostUnit = bTotalCostUnit * 1000;
    }
    ...
    if (targetUsdNum > 0 && exrateNum > 0) {
      ...
      const targetInrTotalKg = (targetBeforeCommission * exrateNum) / unitMultiplier;
      ...
      reverseHtmlOutput = `Required Ex-Mill ${baseCurrency}: ${getCurrencySymbol(baseCurrency)} ${Math.max(rExmill, 0).toFixed(2)} / KG`;
    }
    ...
    if ((industry as any) === "grain") {
      ...
      primaryOutputLabel = "Agri-Commodity Quoted Rate";
    } else if ((industry as any) === "oil") {
      primaryOutputLabel = "Oil Quoted Rate";
    }
  } else {
    // GENERIC MODE
    unitLabel = "UNIT";
    ...
    if (targetUsdNum > 0 && exrateNum > 0) {
      ...
      const otherCosts =
        ...
        freightUnitInr;
      const rExmill = targetBaseCost - otherCosts;
      reverseHtmlOutput = `Required Cargo Ex-Mill ${baseCurrency}: ${getCurrencySymbol(baseCurrency)} ${Math.max(rExmill, 0).toFixed(4)} / UNIT`;
    }
  ...
  const handleSaveToRateList = () => {
    ...
    if (
      ...
      item = {
        ...
        dest: port,
        commodity,
        brand: brand.trim().toUpperCase() || `VNP-${(industry as any).toUpperCase()}`,
        packed: bagtype,
        size: `${(industry as any) === "grain" ? "GRAIN" : (industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()) ? "OIL/LIQUID" : (industry as any).toUpperCase()} ${bagsize} ${sizeUnit}`,
        master: masterbag,
        crop: cropAge,
        year: cropYear || "2025",
        ...
        oilPricingMode,
        oilUseBox,
        oilPiecesPerBox: parseFloat(oilPiecesPerBox) || undefined,
        oilBoxPrice: parseFloat(oilBoxPrice) || undefined,
        manualBottles: manualBottles || undefined,
        totalBottlesPerFcl: (isOilMode || (bagtype || "").includes("BOTTLE") || (bagtype || "").toUpperCase().includes("BOTTLE"))
          ? (parseFloat(manualBottles) || Math.round((parseFloat(fclWeight) || 26000) / (parseFloat(bagsize) || 1)))
          : undefined,
        totalBoxesPerFcl: (isOilMode || (bagtype || "").includes("BOTTLE") || (bagtype || "").toUpperCase().includes("BOTTLE")) && (parseFloat(oilPiecesPerBox) > 0)
          ? Math.round((parseFloat(manualBottles) || ((parseFloat(fclWeight) || 26000) / (parseFloat(bagsize) || 1))) / parseFloat(oilPiecesPerBox))
          : undefined,
        palletEnabled,
        boxesPerPallet: parseFloat(boxesPerPallet) || undefined,
        palletsPerFcl: parseFloat(palletsPerFcl) || undefined,
        palletPackingInr: parseFloat(palletCostInr) || undefined,
        ratePerKl: isOilMode
          ? oilPricingMode === "PIECE"
            ? rateToSave * (1000 / (parseFloat(bagsize) || 1))
            : rateToSave
          : undefined,
    ...
    // FCL capacity validation for agri_multi
    if ((industry as any) === "agri_multi") {
      const totalWeightAddedTons = item.totalWeightKg / 1000;
      const fclCapacityMinTons = item.numFCL * 24;
      const fclCapacityMaxTons = item.numFCL * 26;

      if (totalWeightAddedTons < fclCapacityMinTons) {
        // Show toast mechanism via alert or window.confirm since it's blocking
        const confirmed = window.confirm(`The total weight (${totalWeightAddedTons} MT) is less than the typical capacity for ${item.numFCL} FCL(s) (${fclCapacityMinTons}-${fclCapacityMaxTons} MT). You can add a few more tons.\n\nDo you want to continue saving this product?`);
        if (!confirmed) {
          return;
        }
      }
  ...
  useEffect(() => {
    if (backfillItem && backfillItem.data) {
      ...
      if (d.dest) {
        setPort(d.dest);
        if (d.dest.includes(" - ")) setSelectedCountry(d.dest.split(" - ")[0]);
      }
...
<button
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-3 gap-5">
        ...
        <div className="2xl:col-span-2 space-y-4 font-sans">
          ...
          <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs">
            ...
            (industry as any) === "packaging" ||
                ...
                (industry as any) === "nuts" ||
                (industry as any) === "pharma" ||
                (industry as any) === "oil" || (industry as any) === "agri_multi") && (
                <div className="field">
                  <div className="flex justify-between items-center mb-1">
                ...
                >
                  ...
                  {commodities
                    .filter((c) => typeof c === "string" || !c.industry || c.industry === industry)
                    .filter((c) => {
                       // Enforce strict commodities for agri_multi
                       if (industry === "agri_multi") {
                         const n = (typeof c === "string" ? c : c.name).toUpperCase();
                         return ["RICE", "SUGAR", "PALM OIL", "SUNFLOWER OIL", "MAIZE", "SOYBEAN"].includes(n);
                       }
                       return true;
                    })
                    .map((c) => (
                  ...
                  <div className="flex flex-wrap items-center mt-1 text-[10.5px] gap-y-2 gap-x-4">
                    <span className="fc empty">
                      Prefills ex-mill rate automatically.
                    </span>
                    {(industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()) && (
                      <div className="flex items-center gap-2 mt-2 col-span-2 bg-indigo-50/50 p-2 rounded border border-indigo-100">
                         <span className="text-[10px] font-bold text-indigo-800 uppercase">Pricing Basis:</span>
                         <label className="flex items-center gap-1 text-[10px] cursor-pointer">
                           <input type="radio" name="oilPricing" checked={oilPricingMode === "MT"} onChange={() => setOilPricingMode("MT")} />
                           Per MT / KL
                         </label>
                         <label className="flex items-center gap-1 text-[10px] cursor-pointer">
                           <input type="radio" name="oilPricing" checked={oilPricingMode === "PIECE"} onChange={() => setOilPricingMode("PIECE")} />
                           Per Piece (Bottle/Can)
                         </label>
                      </div>
                    )}
                    {commodity && (
                ...
                <div className="col-span-1 md:col-span-2 bg-[#f0f9ff]/60 border border-blue-200 p-4.5 rounded-xl space-y-3.5 shadow-xs">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 border-b border-blue-200/50 pb-2.5">
                    ...
                    <span className="text-[10px] font-mono font-black text-blue-800 bg-blue-100 px-3 py-1 rounded-full">
                      Calc Ex-Mill Rate: {getCurrencySymbol(baseCurrency)} {computedBlendExMill.toFixed(2)} / KG
                    </span>
                  </div>
              ...
              <div className="field relative">
                <label className="flex justify-between items-center w-full mb-1">
                  <span>Destination Port</span>
                  {setPorts && (
                    <button
                      type="button"
                      onClick={() => setIsAddingPort(!isAddingPort)}
                      className="text-[9px] font-black uppercase text-blue-600 hover:text-blue-800 flex items-center gap-0.5 cursor-pointer border border-blue-200 bg-blue-50 px-1.5 py-0.5 rounded"
                    >
                      {isAddingPort ? <X className="w-3 h-3" /> : <Plus className="w-3 h-3" />} {isAddingPort ? "Close" : "Add New Port"}
                    </button>
                  )}
                </label>
                  ...
                  <label className="flex justify-between items-center">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        // Optionally update default weights based on container size if needed
                        if (val === "40ft" && activeCargoWeightKg === 26000) {
                          setFclWeight("28000"); // 28MT default for 40ft
                        } else if (val === "20ft" && activeCargoWeightKg === 28000) {
                          setFclWeight("26000"); // 26MT default for 20ft
                        }
                      }}
          ...
          {((industry as any) === "grain" ||
            ...
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              <div className="flex items-center justify-between pb-2 border-b mb-3">
                <h3 className="card-title text-[10px] uppercase font-bold text-indigo-600 flex items-center gap-1.5 m-0">
                ...
                {(industry as any) === "agri_multi" && (/\bOIL\b/.test(commodity.toUpperCase()) ? "Oil Commodity Specifications" : "Agri Commodity Specifications")}
              
                </h3>
                {((industry as any) === "oil" || ((industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()))) && (
                   <div className="flex items-center gap-3 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
                     <span className="text-[10px] font-bold text-indigo-800 uppercase">Pricing Basis:</span>
                     <label className="flex items-center gap-1 text-[10px] cursor-pointer font-sans normal-case text-slate-700">
                       <input type="radio" name="oilPricing2" checked={oilPricingMode === "MT"} onChange={() => setOilPricingMode("MT")} />
                       Per MT / KL
                     </label>
                     <label className="flex items-center gap-1 text-[10px] cursor-pointer font-sans normal-case text-slate-700">
                       <input type="radio" name="oilPricing2" checked={oilPricingMode === "PIECE"} onChange={() => setOilPricingMode("PIECE")} />
                       Per Piece (Bottle/Can)
                     </label>
                   </div>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                <div className="field">
                  <label>
                    {(industry as any) === "grain" && "Ex-Mill Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "spices" && "Ex-Factory Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "chemicals" && "Ex-Works Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "salts" && "Ex-Silo Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "vegetables_fruits" &&
                      "Packhouse Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "air_cargo" &&
                      "Ex-Warehouse Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "apparel" &&
                      "Ex-Factory Price ({getCurrencySymbol(baseCurrency)} / Piece or KG)"}
                    {(industry as any) === "packaging" &&
                      "Ex-Factory Price ({getCurrencySymbol(baseCurrency)} / Bag or KG)"}
                    {(industry as any) === "sugar" && "Ex-Mill Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "oil" && "Refinery Price ({getCurrencySymbol(baseCurrency)} / KG or Ltr)"}
                    {(industry as any) === "pharma" && "Factory Price ({getCurrencySymbol(baseCurrency)} / KG or Unit)"}
                    {(industry as any) === "nuts" && "Factory Price ({getCurrencySymbol(baseCurrency)} / KG)"}
                    {(industry as any) === "metal" && `Finished Product Cost (${getCurrencySymbol(baseCurrency)} / KG)`}
                    {(industry as any) === "agri_multi" && (/\bOIL\b/.test(commodity.toUpperCase()) ? "Refinery Price ({getCurrencySymbol(baseCurrency)} / L)" : "Ex-Mill Price ({getCurrencySymbol(baseCurrency)} / KG)")}
                  </label>
                  <div className="relative">
                    <span className="absolute left-2.5 top-2.5 text-gray-400 font-bold text-[10px]">{getCurrencySymbol(baseCurrency)}</span>
                    <input
                      type="number"
                      step="0.01"
                      value={
                ...
                <div className="field">
                  <label>
                    ...
                    {(industry as any) === "agri_multi" && (/\bOIL\b/.test(commodity.toUpperCase()) ? "Oil Brand Label" : "Agri Brand Label")}
                  </label>
                  <input
                    type="text"
                    value={brand}
                ...
                {( ((industry as any) === "grain") || ((industry as any) === "agri_multi" && !/\bOIL\b/.test(commodity.toUpperCase())) ) && (
                  <div className="field">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
                <div className="field">
                  ...
                  >
                    {(
                      (industry as any) === "agri_multi"
                        ? (/\bOIL\b/.test(commodity.toUpperCase())
                            ? ["PET BOTTLES", "GLASS BOTTLES", "TIN CANS", "HDPE JERRY CANS", "FLEXITANKS", "STEEL DRUMS", "IBC TOTES"]
                            : indConfig.categories.filter(c => !["PET BOTTLES", "GLASS BOTTLES", "TIN CANS", "HDPE JERRY CANS", "FLEXITANKS", "IBC TOTES", "STEEL DRUMS"].includes(c)))
                        : indConfig.categories
                    ).map((cat) => (
                ...
                <div className="field">
                  <label>{(industry as any) === "agri_multi" && bagtype.includes("BOTTLE") ? "Bottle Volume" : indConfig.sizeLabel}</label>
                  <div className="flex gap-2">
                    {(() => {
                      const opts = bagPrices[bagtype] || indConfig.defaultSizes[bagtype] || [];
                      if (opts.length > 0) {
                        return (
                          <select
                            value={bagsize}
                    ...
                    <select
                      ...
                      className="w-24 bg-slate-50 border border-slate-200 rounded-xl px-2 py-2 text-[10px] font-bold outline-none text-slate-600"
                    >
                ...
                <div className="field">
                  <label>{(industry as any) === "agri_multi" && bagtype.includes("BOTTLE") ? `Cost (${getCurrencySymbol(baseCurrency)} / Bottle)` : indConfig.costLabel}</label>
                  <input
                    type="number"
                    step="0.01"
                    value={bagcost}
                ...
                <div className="field">
                  <div className="flex justify-between items-center mb-1">
                    <label className="truncate text-slate-500 text-[11px] mb-0">Total Packages (Est.)</label>
                    {(((industry as any) === "oil" || ((industry as any) === "agri_multi" && (bagtype || "").includes("BOTTLE")) || (bagtype || "").toUpperCase().includes("BOTTLE"))) && (
                      <span className="text-[9px] text-indigo-600 font-bold hover:underline cursor-pointer">Click to apply</span>
                    )}
                  </div>
                  <div 
                    onClick={() => {
                      if (parseFloat(fclWeight) > 0 && parseFloat(bagsize) > 0) {
                        const est = Math.floor(parseFloat(fclWeight) / parseFloat(bagsize));
                        setManualBottles(String(est));
                      }
                    }}
                    className="w-full bg-slate-50 hover:bg-indigo-50/50 border border-slate-200 hover:border-indigo-200 rounded-xl px-3 py-2 text-xs font-mono font-black text-center text-slate-700 outline-none cursor-pointer transition-all"
                    title="Click to apply this estimate as the No. of Bottles override"
                  >
                ...
                {/* Manual Bottles Override Entry */}
                {(((industry as any) === "oil" || ((industry as any) === "agri_multi" && (bagtype || "").includes("BOTTLE")) || (bagtype || "").toUpperCase().includes("BOTTLE"))) && (
                  <div className="field">
                    <label className="truncate text-indigo-700 font-bold text-[11px] flex items-center gap-1">
                      <span>No. of Bottles / FCL</span>
                      <span className="text-[9px] font-normal text-slate-400 font-sans tracking-tight">(Optional Entry)</span>
                    </label>
                    <input
                      id="manual-bottles-input"
                      type="number"
                      value={manualBottles}
                      onChange={(e) => setManualBottles(e.target.value)}
                      className="w-full bg-slate-50 hover:bg-white border border-indigo-200 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs font-mono font-bold text-center outline-none transition-colors text-indigo-950 placeholder-indigo-300"
              ...
              {/* Bottle payload validation warning */}
              {(() => {
                const fclWeightNum = parseFloat(fclWeight) || 0;
                const bottleSizeNum = parseFloat(bagsize) || 0;
                const enteredBottlesNum = parseFloat(manualBottles) || 0;
                const isBottleMode = ((industry as any) === "oil" || ((industry as any) === "agri_multi" && (bagtype || "").includes("BOTTLE")) || (bagtype || "").toUpperCase().includes("BOTTLE"));
                
                if (isBottleMode && enteredBottlesNum > 0 && bottleSizeNum > 0 && fclWeightNum > 0) {
                  const calculatedPayloadWeight = enteredBottlesNum * bottleSizeNum;
                  const diff = calculatedPayloadWeight - fclWeightNum;
                  if (Math.abs(diff) > 500) {
                    const recommendedCount = Math.round(fclWeightNum / bottleSizeNum);
                    return (
                      <div className="my-3 bg-red-50/90 border border-red-500/30 rounded-xl p-3.5 text-red-950 shadow-md animate-pulse duration-1000">
                        <div className="flex items-center gap-1.5 font-black text-red-700 text-xs uppercase tracking-wider">
                          <span>🚨</span>
                          <span>CARGO PAYLOAD DEVIATION DETECTED</span>
                        </div>
                        <p className="mt-1.5 text-[11.5px] font-medium leading-relaxed text-red-900">
                          Your entered bottle count of <span className="font-bold text-red-700">{enteredBottlesNum.toLocaleString()} bottles</span> with unit size of <span className="font-bold text-red-700">{bottleSizeNum} {sizeUnit}</span> equates to a total cargo weight of <span className="font-bold text-red-700">{calculatedPayloadWeight.toLocaleString()} KG/L</span>. 
                          This deviates from your declared Cargo Payload Weight (<span className="font-bold text-slate-700">{fclWeightNum.toLocaleString()} KG/L</span>) by <span className="font-bold text-red-700">{Math.abs(diff).toLocaleString()} KG/L</span>.
                        </p>
                        <div className="mt-2.5 flex flex-wrap items-center gap-2">
                          <span className="text-[10px] font-bold text-red-800 bg-red-100 px-2 py-0.5 rounded border border-red-200">
                            Flagged: Check required! (+/- {Math.abs(diff).toLocaleString()} {sizeUnit})
                          </span>
                          <button
                            type="button"
                            onClick={() => {
                              setManualBottles(String(recommendedCount));
                            }}
                            className="bg-red-600 hover:bg-red-700 active:bg-red-800 text-white text-[10.5px] font-black px-3 py-1 rounded-lg transition-all cursor-pointer shadow-sm flex items-center gap-1 hover:scale-105"
                          >
                            <span>⚡</span> Auto-Correct to {recommendedCount.toLocaleString()} Bottles
                          </button>
                        </div>
                      </div>
                    );
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 border-t border-gray-100 pt-3 mt-1">
                ...
                {masterbag === "YES" && (
                  <>
                    <div className="field">
                      <label>Master Bag Price ({getCurrencySymbol(baseCurrency)})</label>
                      <input
                        type="number"
                        step="0.01"
                    ...
                    <div className="field flex flex-col justify-end">
                      <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-2 text-[9px] font-bold text-emerald-800 text-center uppercase shadow-xs">
                        Extra Cost/KG: {getCurrencySymbol(baseCurrency)}{" "}
                        {(
              ...
              {/* Oil Box Packing Module */}
              {(((industry as any) === "oil" || ((industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase())))) && oilPricingMode === "PIECE" && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 border-t border-gray-100 pt-3 mt-1">
                <div className="field">
                  <label>Box Packing</label>
                  <select
                    value={oilUseBox}
                    onChange={(e) => setOilUseBox(e.target.value)}
                    className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2 text-xs font-bold outline-none"
                  >
                    <option value="NO">NO BOXES</option>
                    <option value="YES">YES - PACK IN BOXES</option>
                  </select>
                </div>
                {oilUseBox === "YES" && (
                  <>
                    <div className="field">
                      ...
                      <input
                        type="number"
                        value={oilPiecesPerBox}
                        onChange={(e) => setOilPiecesPerBox(e.target.value)}
                        className="w-full bg-slate-50 border border-gray-300 rounded-xl px-3 py-2 text-xs font-mono font-bold text-center outline-none focus:bg-white focus:border-indigo-500"
                      />
                    </div>
                    <div className="field">
                      <label>Box Price ({getCurrencySymbol(baseCurrency)})</label>
                      <input
                        type="number"
                        step="0.01"
                        value={oilBoxPrice}
                        onChange={(e) => setOilBoxPrice(e.target.value)}
                        className="w-full bg-slate-50 border border-gray-300 rounded-xl px-3 py-2 text-xs font-mono font-bold text-center outline-none focus:bg-white focus:border-indigo-500"
                      />
                    </div>
                    <div className="field flex flex-col justify-end">
                      <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-2 text-[9px] font-bold text-emerald-800 text-center uppercase shadow-xs">
                        Extra Cost/Piece: {getCurrencySymbol(baseCurrency)}{" "}
                        {parseFloat(oilPiecesPerBox) > 0
                          ? ((parseFloat(oilBoxPrice) || 0) / parseFloat(oilPiecesPerBox)).toFixed(4)
                          : "0.0000"}
                        <br/>
                        <span className="text-[7.5px] opacity-80">EST BOXES: {parseFloat(oilPiecesPerBox) > 0 && parseFloat(fclWeight) > 0 && parseFloat(bagsize) > 0 ? Math.floor(Math.floor(parseFloat(fclWeight)/parseFloat(bagsize)) / parseFloat(oilPiecesPerBox)).toLocaleString() : 0}</span>
                      </div>
                    </div>
                  </>
                )}
              ...
              {/* Optional Pallet Loading Module for All Modules */}
              {true && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 border-t border-gray-100 pt-3 mt-1">
                <div className="field">
                ...
                {palletEnabled && (
                  <>
                    ...
                    <div className="field">
                      <label>Cost per Pallet ({getCurrencySymbol(baseCurrency)})</label>
                      <input
                        type="number"
                        step="0.01"
                        value={palletCostInr}
                    ...
                    <div className="field flex flex-col justify-end">
                      <div className="bg-blue-50 border border-blue-100 rounded-xl p-2 text-[9px] font-bold text-blue-800 text-center uppercase shadow-xs">
                        ...
                        <br/>
                        <span className="text-[7.5px] opacity-80">Total Pallets Cost: {getCurrencySymbol(baseCurrency)} {((parseFloat(palletsPerFcl) || 0) * (parseFloat(palletCostInr) || 0)).toFixed(2)}</span>
                      </div>
                    </div>
                  </>
                )}
          ...
          {(industry as any) === "tiles" && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div className="field">
                  <label>Ex-Mill SQM Price ({getCurrencySymbol(baseCurrency)})</label>
                  <input
                    type="number"
                    value={exmillSqmInr}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
                <div className="field">
                  <label>Pallet Inward Label Fee ({getCurrencySymbol(baseCurrency)}/SQM)</label>
                  <input
                    type="number"
              ...
              <div className="border-t border-gray-100 pt-3 mt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-5 gap-3">
                <div className="field">
                  <label>{indConfig.packLabel}</label>
                  <select
                    value={bagtype}
                ...
                <div className="field">
                  <label>{(industry as any) === "agri_multi" && bagtype.includes("BOTTLE") ? "Bottle Volume (L)" : indConfig.sizeLabel}</label>
                  <select
                    value={bagsize}
                    onChange={(e) => setBagsize(e.target.value)}
                    className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2 text-xs font-black font-mono outline-none"
          ...
          {(industry as any) === "generic" && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="field">
                  <label>Ex-Mill Unit Price ({getCurrencySymbol(baseCurrency)})</label>
                  <input
                    type="number"
                    value={exmillUnitInr}
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2.5 bg-amber-50/10 p-3 rounded-xl border border-amber-100">
                ...
                <div className="field">
                  <label>Inward Box Cost ({getCurrencySymbol(baseCurrency)}/Box)</label>
                  <input
                    type="number"
                    value={packageBoxInr}
              ...
              <div className="border-t border-gray-200 pt-3 mt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-5 gap-3">
                <div className="field">
                  <label>{indConfig.packLabel}</label>
                  <select
                    value={bagtype}
              ...
              <div className="field">
                <div className="flex justify-between items-center mb-1">
                  <label className="text-[10px] uppercase font-bold text-slate-500 m-0 p-0 block">
                    Exchange ({getCurrencySymbol(baseCurrency)}/$)
                  </label>
                  <button
                    type="button"
                ...
                <div className="flex gap-1 relative">
                  <select 
                    ...
                    onChange={(e) => {
                       ...
                       // We trigger a sync right after change to get the new rate
                       fetchOnlineExchangeRate(true, e.target.value);
                    }}
                    className="bg-slate-100 border border-gray-300 rounded-xl px-1 py-2 text-[10px] font-bold text-slate-700 outline-none w-16"
                  >
          ...
          <div className="card bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-950 border border-indigo-900 text-white p-5 rounded-2xl shadow-xl flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="bg-slate-950/40 p-4 border border-white/5 rounded-xl text-center relative group">
                ...
                {isOilMode && (
                  <div className="mt-2.5 pt-2 border-t border-white/10 text-xs text-slate-300 font-mono flex items-center justify-between">
                    <span className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400">Final CIF (KL equivalent)</span>
                    <span className="font-bold text-cyan-300 text-sm">
                      ${" "}
                      {(oilPricingMode === "PIECE"
                        ? (isOverridden ? parseFloat(overrideRate) || 0 : finalLandedUsdUnit) * (1000 / (parseFloat(bagsize) || 1))
                        : (isOverridden ? parseFloat(overrideRate) || 0 : finalLandedUsdUnit)
                      ).toLocaleString([], {
                        minimumFractionDigits: 2,
                ...
                <div className="text-[10.5px] font-bold text-center mt-2 font-mono text-cyan-300 tracking-wider">
                  ...
                  {bTotalCostUnit.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 text-white">
                ...
                <div 
                  onClick={() => {
                    const el = document.getElementById("manual-bottles-input");
                    if (el) {
                      el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                      el.focus();
                    }
                  }}
                  className="bg-slate-950/30 p-2.5 rounded-xl border border-white/5 flex flex-col justify-between cursor-pointer hover:bg-slate-900/50 hover:border-cyan-500/20 transition-all group"
                  title="Click to quickly update/override bottle count"
                >
                  <span className="text-[8.5px] font-extrabold text-cyan-200/80 group-hover:text-cyan-300 uppercase tracking-widest flex items-center gap-1">
                    {(industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()) && oilPricingMode === "PIECE"
                      ? "TOTAL PIECES / FCL ✏️"
                      : (industry as any) === "grain" ||
                        (industry as any) === "sugar" ||
                        (industry as any) === "oil" ||
                  ...
                  <span className="text-xs font-bold font-mono tracking-wide mt-1 group-hover:text-cyan-200">
                    {(industry as any) === "agri_multi" && /\bOIL\b/.test(commodity.toUpperCase()) && oilPricingMode === "PIECE"
                      ? Math.floor((parseFloat(fclWeight) || 26000) / (parseFloat(bagsize) || 1)).toLocaleString()
                      : (industry as any) === "grain" ||
                        (industry as any) === "sugar" ||
                        (industry as any) === "oil" ||
                        (industry as any) === "metal" ||
              ...
              {/* Liquid / KL Volume & FCL Quote Summary (ONLY FOR OIL/LIQUIDS) */}
              {isOilMode && (() => {
                const isPiece = oilPricingMode === "PIECE";
                const bottleSizeNum = parseFloat(bagsize) || 0;
                const enteredBottlesNum = parseFloat(manualBottles) || 0;
                const totalKlPerFcl = fclWeightNum / 1000;
                const totalBottlesPerFcl = isPiece 
                  ? (enteredBottlesNum || Math.round(fclWeightNum / (bottleSizeNum || 1)))
                  : 0;
                const totalFclPriceUsd = isPiece 
                  ? totalBottlesPerFcl * finalLandedUsdUnit 
                  : totalKlPerFcl * finalLandedUsdUnit;
                const grandContractTotalUsd = numFclNum * totalFclPriceUsd;
                const ratePerKl = totalKlPerFcl > 0 ? totalFclPriceUsd / totalKlPerFcl : 0;
                
                // Check for payload deviation
                let hasDeviationWarning = false;
                let deviationAmount = 0;
                let recommendedCount = 0;
                if (isPiece && enteredBottlesNum > 0 && bottleSizeNum > 0 && fclWeightNum > 0) {
                  const calculatedPayloadWeight = enteredBottlesNum * bottleSizeNum;
                  deviationAmount = calculatedPayloadWeight - fclWeightNum;
                  if (Math.abs(deviationAmount) > 500) {
                    hasDeviationWarning = true;
                    recommendedCount = Math.round(fclWeightNum / bottleSizeNum);
                  }
                }

                return (
                  <div className="mt-4 bg-slate-950/60 p-3.5 border border-cyan-500/30 rounded-xl space-y-2.5 text-xs text-slate-200">
                    <div className="flex items-center justify-between border-b border-white/10 pb-1.5">
                      <div className="flex items-center gap-1.5 font-black text-cyan-300 text-[10px] uppercase tracking-widest">
                        <span>💧</span>
                        <span>Liquid KL Volume & Quote Summary</span>
                      </div>
                      {hasDeviationWarning && (
                        <span className="bg-red-500/30 border border-red-500/50 text-red-300 px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider animate-pulse">
                          ⚠️ Deviation Alert
                        </span>
                      )}
                    </div>
                    ...
                    {hasDeviationWarning && (
                      <div className="bg-red-950/60 border border-red-500/30 rounded-lg p-2.5 space-y-1 text-red-200 text-[11px] font-sans">
                        <div className="font-extrabold text-red-400 flex items-center gap-1">
                          <span>🚨</span> Flagged: Weight deviates from Payload by {Math.abs(deviationAmount).toLocaleString()} KG/L!
                        </div>
                        <div>
                          Payload Weight: <span className="font-bold">{fclWeightNum.toLocaleString()} KG/L</span>. Entered Bottles Weight: <span className="font-bold">{(enteredBottlesNum * bottleSizeNum).toLocaleString()} KG/L</span>.
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="mt-1 w-full bg-red-600 hover:bg-red-700 active:bg-red-800 text-white font-bold py-1 px-2 rounded text-[10px] transition-all cursor-pointer flex items-center justify-center gap-1 hover:scale-102"
                        >
                          <span>⚡</span> Click to Auto-Update to exact {recommendedCount.toLocaleString()} bottles
                        </button>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-medium font-sans">
                      <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                        <div className="text-[9px] text-slate-400 font-extrabold uppercase text-left">Total KL / FCL</div>
                        <div className="font-mono text-cyan-200 font-bold mt-0.5 text-left">
                          {totalKlPerFcl.toFixed(3)} KL
                        </div>
                      </div>
                      
                      <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                        <div className="text-[9px] text-slate-400 font-extrabold uppercase text-left">Number of FCLs</div>
                        <div className="font-mono text-cyan-200 font-bold mt-0.5 text-left">
                          {numFclNum} FCL
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                        <div className="text-[9px] text-slate-400 font-extrabold uppercase text-left">Total KL Volume</div>
                        <div className="font-mono text-emerald-300 font-black mt-0.5 text-xs text-left">
                          {(numFclNum * totalKlPerFcl).toFixed(3)} KL
                        </div>
                      </div>
                      
                      <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                        <div className="text-[9px] text-slate-400 font-extrabold uppercase text-left">Rate per KL</div>
                        <div className="font-mono text-cyan-200 font-bold mt-0.5 text-left">
                          $ {ratePerKl.toFixed(2)}
                        </div>
                      </div>
                    ...
                    <div className="bg-slate-900/80 border border-cyan-500/20 p-2.5 rounded-lg space-y-1.5 font-sans">
                      <div className="flex justify-between items-center text-[10.5px]">
                        <span className="text-slate-400 font-bold">Price / 1 FCL:</span>
                        <span className="font-mono font-bold text-cyan-300 text-[11px]">
                          $ {totalFclPriceUsd.toLocaleString([], { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs border-t border-white/10 pt-1.5">
                        <span className="text-slate-200 font-black">GRAND CONTRACT TOTAL:</span>
                        <span className="font-mono font-black text-emerald-400 text-sm">
                          $ {grandContractTotalUsd.toLocaleString([], { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                        </span>
                      </div>
                      <div className="text-[9px] text-slate-400 font-medium italic text-right leading-relaxed">
                        {isPiece ? (
                          <>
                            {numFclNum} FCL × {totalBottlesPerFcl.toLocaleString()} bottles/FCL = {(numFclNum * totalBottlesPerFcl).toLocaleString()} bottles @ $ {finalLandedUsdUnit.toFixed(3)}/bottle
                            <br />
                            Equivalent: {(numFclNum * totalKlPerFcl).toFixed(2)} KL @ $ {ratePerKl.toFixed(2)}/KL
                          </>
                        ) : (
                          <>{numFclNum} FCL × {totalKlPerFcl.toFixed(2)} KL/FCL = {(numFclNum * totalKlPerFcl).toFixed(2)} KL @ $ {ratePerKl.toFixed(2)}/KL</>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-1.5 text-xs text-slate-700">
              <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                ...
                <span className="font-mono font-bold text-slate-800">
                  {bExMill.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                <span className="font-medium text-slate-500">
                  ...
                  (industry as any) === "oil" ||
                  (industry as any) === "metal"
                    ? `Bag/Drum (${bagsize}kg/L @ {getCurrencySymbol(baseCurrency)}${bagcost}/unit)`
                    : "Inward Pack Price"}
                </span>
                <span className="font-mono font-bold text-slate-800">
                  {bPackaging.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                ...
                <span className="font-mono font-bold text-slate-800">
                  {bFreight.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                ...
                <span className="font-mono font-bold text-slate-800">
                  {bTransport.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                ...
                <span className="font-mono font-bold text-slate-800">
                  {bCfsPort.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                </span>
              </div>
              ...
              {bDuty > 0 && (
                <div className="flex justify-between items-center py-1 border-b border-gray-200/60">
                  ...
                  <span className="font-mono font-bold text-slate-800">
                    {bDuty.toFixed(costUnitLabel === "KL" ? 2 : 4)}/{costUnitLabel === "L" || costUnitLabel === "KL" || costUnitLabel === "MT" ? costUnitLabel : costUnitLabel.toLowerCase()}
                  </span>
                </div>
              )}
          ...
          {isBagStockEnabled && (
            <div className="card bg-white p-4 rounded-xl border border-gray-200 shadow-xs space-y-3">
              <h4 className="text-[10px] uppercase tracking-widest font-black text-slate-500 leading-none">
                {bagtype ? `${bagtype.toUpperCase()} STOCK CHECK` : "PACKAGING STOCK CHECK"}
              </h4>
                  ...
                  return (
                    <>
                      <div className="flex justify-between items-baseline border-b border-gray-100 pb-1">
                        ...
                        >
                          {matched
                            ? `${(matched.stock || 0).toLocaleString()} ${((industry as any) === "agri_multi" && bagtype.includes("BOTTLE")) || ((industry as any) === "oil") ? "Units" : "Bags"}`
                            : "NA"}
                        </span>
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-slate-900/60 p-3.5 rounded-xl border border-slate-800">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    <select
                      value={estSourceCountry}
                      onChange={(e) => {
                        setEstSourceCountry(e.target.value);
                        setEstSource("");
                      }}
                      className="w-1/2 bg-slate-950 border border-slate-800 text-teal-300 font-bold text-[11px] px-2 py-1.5 rounded-lg outline-none focus:border-teal-500 cursor-pointer"
                    >
                      <option value="">Country</option>
                      {countries.map(c => <option key={c} value={c}>{c}</option>)}
                      {!countries.includes("INDIA") && <option value="INDIA">INDIA</option>}
                      {!countries.includes("UAE") && <option value="UAE">UAE</option>}
                      {!countries.includes("CHINA") && <option value="CHINA">CHINA</option>}
                      {!countries.includes("USA") && <option value="USA">USA</option>}
                      {!countries.includes("VIETNAM") && <option value="VIETNAM">VIETNAM</option>}
                    </select>

                    <select
                      value={estSource.includes(" - ") ? estSource.split(" - ")[1] : estSource}
                      onChange={(e) => {
                        if (estSourceCountry && estSourceCountry !== "OTHER") {
                          setEstSource(estSourceCountry + " - " + e.target.value);
                        } else {
                          setEstSource(e.target.value);
                        }
                      }}
                      className="w-1/2 bg-slate-950 border border-slate-800 text-teal-300 font-bold text-[11px] px-2 py-1.5 rounded-lg outline-none focus:border-teal-500 cursor-pointer"
                    >
                      <option value="">Port</option>
                      {(portsByCountry[estSourceCountry] || []).map(p => <option key={p} value={p}>{p}</option>)}
                      
                      {/* Standard fallback ports for India if none are configured yet */}
                      {estSourceCountry === "INDIA" && (
                        <>
                          <option value="MUNDRA">MUNDRA</option>
                          <option value="NHAVA SHEVA (JNPT)">NHAVA SHEVA</option>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    <select
                      value={estDestCountry}
                      onChange={(e) => {
                        setEstDestCountry(e.target.value);
                        setEstDest("");
                      }}
                      className="w-1/2 bg-slate-950 border border-slate-800 text-white font-bold text-[11px] px-2 py-1.5 rounded-lg outline-none focus:border-teal-500 cursor-pointer"
                    >
                      <option value="">Country</option>
                      {countries.map(c => <option key={c} value={c}>{c}</option>)}
                      {!countries.includes("NETHERLANDS") && <option value="NETHERLANDS">NETHERLANDS</option>}
                      {!countries.includes("GERMANY") && <option value="GERMANY">GERMANY</option>}
                      {!countries.includes("UK") && <option value="UK">UK</option>}
                      {!countries.includes("USA") && <option value="USA">USA</option>}
                      {!countries.includes("CANADA") && <option value="CANADA">CANADA</option>}
                    </select>
                    
                    <select
                      value={estDest.includes(" - ") ? estDest.split(" - ")[1] : estDest}
                      onChange={(e) => {
                         if (estDestCountry && estDestCountry !== "OTHER") {
                           setEstDest(estDestCountry + " - " + e.target.value);
                         } else {
                           setEstDest(e.target.value);
                         }
                      }}
                      className="w-1/2 bg-slate-950 border border-slate-800 text-white font-bold text-[11px] px-2 py-1.5 rounded-lg outline-none focus:border-teal-500 cursor-pointer"
                    >
                      <option value="">Port</option>
                      {(portsByCountry[estDestCountry] || []).map(p => <option key={p} value={p}>{p}</option>)}
                      
                      {/* Fallback hubs for rich selection */}
                      {estDestCountry === "NETHERLANDS" && <option value="ROTTERDAM">ROTTERDAM</option>}
                      {estDestCountry === "GERMANY" && <option value="HAMBURG">HAMBURG</option>}
                      {estDestCountry === "UK" && <><option value="LONDON">LONDON</option><option value="FELIXSTOWE">FELIXSTOWE</option></>}
                      {estDestCountry === "USA" && <><option value="NEW YORK">NEW YORK</option><option value="HOUSTON">HOUSTON</option><option value="LOS ANGELES">LOS ANGELES</option></>}
                      {estDestCountry === "CANADA" && <option value="VANCOUVER">VANCOUVER</option>}
                    </select>
                  </div>
                </div>

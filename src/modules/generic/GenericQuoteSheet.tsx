...
export default function GenericQuoteSheet({
  selectedRateIds,
  rateList,
  savedQuotes,
  setSavedQuotes,
  ...
  const [tableColumns, setTableColumns] = useState([
    ...
    {
      id: "rate",
      label: `Rate / ${(industry as any) === "tiles" ? "SQM" : (industry as any) === "oil" ? "KL" : "MT"}`,
      visible: true,
    },
  ]);
  ...
  // Multi-agri capacity toast
  useEffect(() => {
    if ((industry as any) === "agri_multi" && chosenRates.length > 0) {
      const totalWeightKg = chosenRates.reduce((acc, row) => acc + (row.totalWeightKg || 0), 0);
      const totalWeightTons = totalWeightKg / 1000;
      const fclsNeeded = Math.max(1, Math.ceil(totalWeightTons / 26));
      const minWeight = fclsNeeded * 24;
      
      if (totalWeightTons < minWeight) {
        showToast(`Selected products (${totalWeightTons.toFixed(2)} MT) do not reach FCL capacity (${minWeight}-${fclsNeeded*26} MT). Please add more.`, true);
      }
    }
  }, [chosenRates.length, industry]); // only re-run when number of chosen rates changes
  ...
  const handleSaveQuote = () => {
    ...
    // Calculate if items are under FCL capacity for agri_multi
    if ((industry as any) === "agri_multi") {
      ...
      const fclsNeeded = Math.max(1, Math.ceil(totalWeightTons / 26));
      const minWeight = fclsNeeded * 24;
      const maxWeight = fclsNeeded * 26;

      if (totalWeightTons < minWeight) {
        const confirmed = window.confirm(
          `The total weight of selected products (${totalWeightTons.toFixed(2)} MT) does not reach the typical capacity for ${fclsNeeded} FCL(s) (${minWeight}-${maxWeight} MT). \n\nAre you sure you want to proceed and save this quote?`
        );
        if (!confirmed) return;
      }
    }
    ...
    const items: PIItem[] = chosenRates.map((r, index) => ({
      ...
      industry: r.industry,
      oilPricingMode: r.oilPricingMode,
      oilUseBox: r.oilUseBox,
      oilPiecesPerBox: r.oilPiecesPerBox,
      oilBoxPrice: r.oilBoxPrice,
      totalBottlesPerFcl: r.totalBottlesPerFcl,
      manualBottles: r.manualBottles,
      totalBoxesPerFcl: r.totalBoxesPerFcl,
      palletEnabled: r.palletEnabled,
      boxesPerPallet: r.boxesPerPallet,
      palletsPerFcl: r.palletsPerFcl,
      palletPackingInr: r.palletPackingInr,
    }));
                  ...
                  const renderCellContent = (colId: string) => {
                    switch (colId) {
                      ...
                      case "noOfFcl":
                        return <>{row.numFCL || 1}</>;
                      case "totalWeight": {
                        const isOilItem = (industry as any) === "oil" || row.commodity?.toUpperCase().includes("OIL");
                        const isPiece = row.oilPricingMode === "PIECE" || row.packed?.toUpperCase().includes("BOTTLE");
                        const totalKg = row.totalWeightKg || ((row.numFCL || 1) * (row.weightPerContainerKg || 26000));
                        return (
                          <>
                            {totalKg.toLocaleString()} {(industry as any) === "tiles" ? "BOXES" : "KG"}
                            {isOilItem && !isPiece && (
                              <div className="text-[0.8em] font-black text-emerald-600 font-sans mt-0.5 whitespace-nowrap">
                                Equivalent: {(totalKg / 1000).toFixed(2)} KL
                              </div>
                            )}
                          </>
                        );
                      ...
                      case "rate": {
                        const isOilItem = (industry as any) === "oil" || row.commodity?.toUpperCase().includes("OIL");
                        const isPiece = row.oilPricingMode === "PIECE" || row.packed?.toUpperCase().includes("BOTTLE");
                        const bottleSizeNum = parseFloat(row.size.replace(/[^\d.]/g, '')) || 1;
                        const finalRatePerKl = row.ratePerKl || (isPiece ? row.rate * (1000 / bottleSizeNum) : row.rate);
                        const perBottlePrice = isPiece
                          ? row.rate
                          : finalRatePerKl / (1000 / bottleSizeNum);
                        ...
                        if (isOilItem) {
                          return (
                            <>
                              ...
                              >
                                USD{" "}
                                {finalRatePerKl.toLocaleString([], {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                                })}
                                <span className="text-[0.7em] font-normal text-gray-400 pl-1 uppercase">
                                  /KL
                                </span>
                              </div>
                              <div className="text-[0.8em] font-medium font-mono text-left text-indigo-700 mt-1">
                                (${perBottlePrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} / bottle)
                              </div>
                        ...
                        const itemUnit = (industry as any) === "tiles" ? "SQM" : "MT";
                        return (
                          <>
                            <div
                              className={`font-mono font-bold ${theme.text} text-[1.15em]`}
                            ...
                            <div className="mt-1 flex items-center justify-end gap-1">
                              <span className="text-[9px] font-bold text-gray-400">/ {itemUnit}</span>
                              <span
                                className={`text-[0.75em] font-mono font-bold ${theme.text} ${theme.divide.replace("divide-", "bg-")} border ${theme.ring.replace("ring-", "border-")} px-1.5 py-0.5 rounded uppercase`}
                              >
                                {row.condition || "CIF"}
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="text-[10px] font-extrabold text-blue-950 flex justify-between">
                                          <span>
                                            {ratio.r1}% {r1Name.split(" ")[0]}
                                          </span>
                                          <span className="text-gray-400 font-normal">
                                            /
                                          </span>
                                          <span>
                                            {ratio.r2}% {r2Name.split(" ")[0]}
                                          </span>
                                        </div>
                                        ...
                                        <div className="flex items-baseline justify-between mt-1 pt-1 border-t border-blue-100/50 font-mono">
                                          ...
                                          <span className="text-[10.5px] font-black text-blue-900 text-right">
                                            ...
                                            {isRowActiveSelection && (
                                              <span className="text-[8px] text-blue-700 ml-1 font-sans font-black">
                                                ★
                                              </span>
                                            )}
                                          </span>

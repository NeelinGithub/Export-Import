...
function getProcessTypeFromName(
  name: string,
):
  | "steam"
  | "parboiled"
  ...
  if (norm.includes("basmati") && norm.includes("sella"))
    return "sella_basmati";
  if (norm.includes("golden sella") || norm.includes("golden_sella"))
    return "golden_sella";
  if (norm.includes("creamy sella") || norm.includes("creamy_sella"))
    return "creamy_sella";
  if (norm.includes("steam")) return "steam";
  if (norm.includes("parboiled") || norm.includes("parboil"))
    return "parboiled";
...
function getProcessTypeName(type: string): string {
  switch (type) {
    case "steam":
      return "Steam Rice";
    case "parboiled":
      return "Parboiled Rice";
    case "sella_basmati":
      return "Sella Basmati";
    case "golden_sella":
      return "Golden Sella";
    case "creamy_sella":
      return "Creamy Sella";
...
export default function InventoryManager({
  ...
  // Compute industry label for generic UI strings
  const industryLabel =
      ...
      : (industry as any) === "vegetables_fruits"
        ...
        : (industry as any) === "tiles"
          ...
          : (industry as any) === "metal"
            ? "Smelting & Casting"
            : (industry as any) === "pharma"
              ? "Synthesis & Blistering"
              : (industry as any) === "oil"
                ? "Refining & Packing"
                : "Processing";

  const industryFilteredInventory = grainInventory.filter(
    (i) => (i.industry || "grain") === industry,
  );
  const industryFilteredOrders = inventoryOrders.filter(
    (o) => (o.industry || "grain") === industry,
  );

  // Current active sub-tab: 'sheets' or 'orders' or 'rfq'
  const [subTab, setSubTab] = useState<"balance" | "orders" | "analyzer">(
    "balance",
  );
  ...
  const [orderType, setOrderType] = useState<"Paddy" | "Processed Grain">(
    "Paddy",
  );
  ...
  useEffect(() => {
    if (commodities.length > 0 && industryFilteredInventory.length === 0) {
      ...
      setGrainInventory([...grainInventory, ...initialInv]);
    }
  }, [commodities, grainInventory, industry]);
  ...
  const handleAddGrainRow = (e: React.FormEvent) => {
    ...
    const alreadyExists = grainInventory.some(
      (g) => g.grainName.toLowerCase() === newGrainName.toLowerCase(),
    );
    if (alreadyExists) {
      showToast(
        "This grain selection is already configured in inventory.",
        "warn",
      );
      return;
    }
    ...
    setNewGrainName("");
    showToast(
      `Added ${newRow.grainName} (${getProcessTypeName(processType)}) to mill inventory rosters.`,
    );
  };
      ...
      grainInventory.map((item) => {
        if (item.id === id) {
          ...
          if (key === "paddyBagsCount") {
            ...
            updated.paddyStockTons = Number(
              ((bags * 60) / 1000 + siloT).toFixed(3),
            );
            ...
            const siloT = item.paddySiloTons || 0;
            updated.paddyBagsCount = Math.max(
              0,
              Math.round(((tons - siloT) * 1000) / 60),
            );
          } else if (key === "processedRiceBagsCount") {
            const bags = parseInt(value) || 0;
            const siloT = item.processedSiloTons || 0;
            updated.processedRiceTons = Number(
              ((bags * 50) / 1000 + siloT).toFixed(3),
            );
            ...
            const tons = parseFloat(value) || 0;
            const siloT = item.processedSiloTons || 0;
            updated.processedRiceBagsCount = Math.max(
              0,
              Math.round(((tons - siloT) * 1000) / 50),
            );
  ...
  const handleRemoveItem = (id: string) => {
    if (
      !confirm(
        "Are you sure you want to stop tracking inventory for this item?",
      )
    )
      return;
  ...
  const handlePlaceOrder = (e: React.FormEvent) => {
    ...
    showToast(
      `Placed order for ${qtyTons} Tons of ${orderGrainName}! Expected delivery in ${lead} days.`,
      "success",
    );
    setOrderSupplier("");
  };
  ...
  const handleMarkReceived = (order: InventoryOrder) => {
    setGrainInventory(
      grainInventory.map((item) => {
        if (
          item.grainName.trim().toUpperCase() ===
          order.grainName.trim().toUpperCase()
        ) {
          if (order.type === "Paddy") {
            const nextTons = item.paddyStockTons + order.orderQtyTons;
            const siloT = item.paddySiloTons || 0;
            const nextBags = Math.max(
              0,
              Math.round(((nextTons - siloT) * 1000) / 60),
            );
            return {
              ...item,
            ...
            const nextBags = Math.max(
              0,
              Math.round(((nextTons - siloT) * 1000) / 50),
            );
            return {
              ...item,
    ...
    showToast(
      `Successfully received ${order.orderQtyTons} Tons of ${order.grainName}. Mill balances adjusted.`,
      "success",
    );
  };
  ...
  const analyzeAvailability = () => {
    ...
    if (testerQuoteId !== "custom") {
      const selectedQuote = savedQuotes.find(
        (q) => String(q.id) === testerQuoteId,
      );
    ...
    if (!commodityName || targetTons <= 0) {
      return {
        status: "error",
        message:
          "Enter a valid commodity and weight target to start simulation",
      };
    }
    ...
    const info =
      industryFilteredInventory.find(
        (g) =>
          g.grainName.trim().toUpperCase() ===
            commodityName.trim().toUpperCase() ||
          commodityName
            .trim()
            .toUpperCase()
            .includes(g.grainName.trim().toUpperCase()) ||
          g.grainName
            .trim()
            .toUpperCase()
            .includes(commodityName.trim().toUpperCase()),
      ) || industryFilteredInventory[0];

    if (!info) {
      return {
        status: "warning",
        message:
          "No exact inventory profile details found for this variety. Simulating using generic fallbacks.",
      };
    }
    ...
    const processedRiceDeficit = Math.max(
      0,
      riceNeededForDeficit - availableProcessedRiceTons,
    );

    // 3. Raw Paddy check (to mill the processed rice deficit)
    const paddyNeededForDeficit =
      processedRiceDeficit > 0 ? processedRiceDeficit / millingYield : 0;
    const availableRawPaddyTons = info.paddyStockTons;
    const paddyDeficit = Math.max(
      0,
      paddyNeededForDeficit - availableRawPaddyTons,
    );
    ...
    let isPaddyFitted =
      !isInstantReady && !isProcessedRiceFitted && paddyDeficit <= 0;
      ...
      <div className="bg-gradient-to-r from-emerald-800 to-indigo-950 rounded-2xl p-6 text-white shadow-md relative overflow-hidden">
        ...
        <div className="max-w-xl space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/20 rounded-full text-xs font-bold tracking-wide uppercase">
            ...
            <span>{industryProcessName} Core Activity Center</span>
          </div>
          <h2 className="text-2xl font-black tracking-tight font-sans">
            {industryLabel} Inventory & Production Control
          </h2>
          <p className="text-xs text-emerald-100 leading-relaxed font-sans max-w-sm">
            Control raw silos, milled stocks, and packaging lead times. Evaluate
            your actual capacity against active global client RFQs in real-time.
          </p>
        </div>
      ...
      <div className="flex border-b border-gray-200 gap-6">
        <button
          ...
          className={`pb-3 text-xs uppercase tracking-wider font-extrabold transition-all border-b-2 hover:text-slate-900 cursor-pointer ${
            subTab === "balance"
              ? "border-emerald-600 text-emerald-600 font-sans font-black"
              : "border-transparent text-gray-500 font-sans font-semibold"
          }`}
        >
          {industryLabel} Stock Balances
        </button>
        <button
        ...
        >
          {(industry as any) === "grain"
            ? "Paddy Purchase Orders"
            : "Supply Purchase Orders"}{" "}
          ({industryFilteredOrders.length})
        </button>
        <button
      ...
      {subTab === "balance" && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 bg-white border border-gray-100 rounded-2xl flex items-center gap-3 shadow-xs">
              ...
              <div>
                <span className="text-[10px] text-gray-400 uppercase font-mono tracking-widest block">
                  Raw / Unprocessed Stock
                </span>
                <span className="text-lg font-black font-sans text-gray-950">
                  {industryFilteredInventory
                    .reduce(
                      (acc, curr) => acc + (Number(curr.paddyStockTons) || 0),
                      0,
                    )
                    .toFixed(0)}{" "}
                  {(industry as any) === "grain" ? "Tons" : "Units"}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 bg-white border border-gray-100 rounded-2xl flex items-center gap-3 shadow-xs">
              ...
              <div>
                <span className="text-[10px] text-gray-400 uppercase font-mono tracking-widest block">
                  Finished / Processed Stock
                </span>
                <span className="text-lg font-black font-sans text-gray-950">
                  {industryFilteredInventory
                    .reduce(
                      (acc, curr) =>
                        acc + (Number(curr.processedRiceTons) || 0),
                      0,
                    )
                    .toFixed(0)}{" "}
                  {(industry as any) === "grain" ? "Tons" : "Units"}
            ...
            <div className="p-4 bg-white border border-gray-100 rounded-2xl flex items-center gap-3 shadow-xs">
              ...
              <div>
                <span className="text-[10px] text-gray-400 uppercase font-mono tracking-widest block">
                  Stitched Bag Inventory
                </span>
                <span className="text-lg font-black font-sans text-gray-950">
                  {industryFilteredInventory
                    .reduce(
                      (acc, curr) => acc + (Number(curr.readyBagsCount) || 0),
                      0,
                    )
                    .toLocaleString()}{" "}
                  {(industry as any) === "tiles" ? "Pallets" : "Bags"}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap gap-4 items-center justify-between bg-gray-50/50">
              <div className="space-y-1">
                <h3 className="text-sm font-black uppercase text-gray-900">
                  Active Silo Stock & Lead Time Configurations
                </h3>
                <p className="text-[10px] text-gray-500 font-sans">
                  {(industry as any) === "grain"
                    ? "Directly modify stock amounts (Paddy, Polished Rice, Bags) and the supplier/milling lead time limits."
                    : "Directly modify stock amounts (Raw materials, processed units) and the supplier processing lead time limits."}
                </p>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-100 text-[9px] font-black uppercase tracking-wider text-gray-500 border-b border-gray-200">
                          ...
                          : (industry as any) === "metal"
                            ...
                            : (industry as any) === "pharma"
                              ...
                              : (industry as any) === "oil"
                                ...
                                : "Commodity Varietys & Grading"}
                    </th>
                    <th className="py-3 px-4 text-center">
                      {(industry as any) === "grain"
                        ? "Paddy Stock (60 kg Jute Bags)"
                        : (industry as any) === "tiles"
                          ? "Raw Materials (Clay/Feldspar)"
                          : (industry as any) === "metal"
                            ? "Raw Scrap/Ore Stock"
                            : (industry as any) === "pharma"
                    ...
                    {showSiloColumns && (
                      <th className="py-3 px-4 text-center text-indigo-800 bg-indigo-50/50">
                        {(industry as any) === "grain"
                          ? "Paddy Silo (Bulk Tons)"
                          : "Raw Bulk (Tons)"}
                      </th>
                    )}
                    <th className="py-3 px-4 text-center">
                      {(industry as any) === "grain"
                        ? "Rice Stock (50 kg PP Bags)"
                        : (industry as any) === "tiles"
                          ? "Finished Tiles (Boxes)"
                          : (industry as any) === "metal"
                            ? "Processed Ingots/Metals"
                            : (industry as any) === "pharma"
                              ? "Finished Drugs (Strips/Vials)"
                              : (industry as any) === "oil"
                                ? "Finished Refined Oil"
                                : "Finished Product (Pack/Box)"}
                    </th>
                    {showSiloColumns && (
                      <th className="py-3 px-4 text-center text-emerald-800 bg-emerald-50/50">
                        {(industry as any) === "grain"
                          ? "Rice Silo (Bulk Tons)"
                          : "Processed Bulk (Tons)"}
                      </th>
                    ...
                    <th className="py-3 px-4 text-center">
                      {(industry as any) === "tiles"
                        ? "Palletized Boxes"
                        : "Custom Order Stitched Bags"}
                    </th>
                    <th className="py-3 px-4">Preferred Supplier</th>
                    <th className="py-3 px-4 text-center">
                      Lead Days (Get/{industryProcessName}/Pack)
                    </th>
                    <th className="py-3 px-4 text-right">Delete</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-xs">
                  {industryFilteredInventory.length === 0 ? (
                    <tr>
                      <td
                        colSpan={showSiloColumns ? 9 : 7}
                        className="py-8 text-center text-gray-400 font-semibold italic"
                      >
                        No processing lines configured yet. Enter a custom
                        product name above or load default commodities.
                      </td>
                    </tr>
                  ) : (
                    industryFilteredInventory.map((item) => {
                      const paddyBags =
                        item.paddyBagsCount !== undefined
                          ? item.paddyBagsCount
                          : Math.max(
                              0,
                              Math.round((item.paddyStockTons * 1000) / 60),
                            );
                      const processedBags =
                        item.processedRiceBagsCount !== undefined
                          ? item.processedRiceBagsCount
                          : Math.max(
                              0,
                              Math.round((item.processedRiceTons * 1000) / 50),
                            );
                      const paddySilo =
                        item.paddySiloTons !== undefined
                          ? item.paddySiloTons
                          : 0;
                      const processedSilo =
                        item.processedSiloTons !== undefined
                          ? item.processedSiloTons
                          : 0;
                      const processType =
                        item.millingProcessType ||
                        getProcessTypeFromName(item.grainName);
                        ...
                        >
                          <td className="py-2.5 px-3 min-w-56">
                            ...
                            <div className="mt-1 flex items-center gap-1 leading-none">
                              <span className="text-[8px] uppercase text-gray-400 font-mono">
                                Process:
                              </span>
                              <select
                                value={processType}
                                ...
                                className="bg-gray-100 border border-gray-200 text-[10px] text-slate-700 font-bold px-1.5 py-0.5 rounded cursor-pointer max-w-min focus:outline-none"
                              >
                                <option value="steam">Steam Rice (3d)</option>
                                <option value="parboiled">
                                  Parboiled (5d)
                                </option>
                                <option value="sella_basmati">
                                  Sella Basmati (8d)
                                </option>
                                <option value="golden_sella">
                                  Golden Sella (7d)
                                </option>
                                <option value="creamy_sella">
                                  Creamy Sella (7d)
                                </option>
                                <option value="other">
                                  Standard/Direct (3d)
                          ...
                          <td className="py-2.5 px-3 text-center w-36">
                            <div className="flex flex-col items-center gap-1 justify-center">
                              <input
                                ...
                                className="w-24 bg-gray-50 border border-gray-200 text-center py-1 rounded text-xs text-amber-950 font-black outline-none focus:bg-white focus:border-amber-500 font-mono"
                              />
                            ...
                            <div className="flex flex-col items-center gap-1 justify-center">
                              <input
                                ...
                                onChange={(e) =>
                                  handleUpdateItem(
                                    item.id,
                                    "processedRiceBagsCount",
                                    parseInt(e.target.value) || 0,
                                  )
                                }
                                className="w-24 bg-gray-50 border border-gray-200 text-center py-1 rounded text-xs text-emerald-950 font-black outline-none focus:bg-white focus:border-emerald-500 font-mono"
                              />
                          ...
                          {showSiloColumns && (
                            <td className="py-2.5 px-3 text-center w-28 bg-emerald-50/20">
                              <input
                                ...
                                className="w-20 bg-gray-50 border border-emerald-200 text-center py-1 rounded text-xs text-emerald-950 font-bold outline-none focus:bg-white focus:border-emerald-500 font-mono"
                              />
                            </td>
                          )}
                          ...
                          <td className="py-2.5 px-3 text-center w-36">
                            <div className="flex items-center gap-1 justify-center">
                              <input
                                ...
                                className="w-16 bg-gray-50 border border-gray-200 text-center py-1 rounded text-xs text-gray-950 font-extrabold outline-none focus:bg-white focus:border-indigo-500 font-mono"
                              />
                          ...
                          <td className="py-2.5 px-3 min-w-44">
                            <input
                              ...
                              className="w-full bg-gray-50 border border-gray-200 px-2 py-1 rounded text-[11px] text-gray-800 outline-none focus:bg-white"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-3 text-center w-40">
                            <div className="flex items-center gap-1.5 justify-center font-mono text-[10px]">
                              <div className="flex flex-col items-center">
                                <span className="text-[7px] text-gray-400 uppercase font-black">
                                  Get
                                </span>
                                <input
                                  type="number"
                                  ...
                                  className="w-8 bg-gray-50 border border-gray-200 text-center rounded text-xs text-amber-700 font-black outline-none focus:bg-white"
                                />
                              </div>
                              ...
                              <div className="flex flex-col items-center">
                                ...
                                <input
                                  ...
                                  className="w-8 bg-gray-50 border border-gray-200 text-center rounded text-xs text-emerald-700 font-black outline-none focus:bg-white font-mono"
                                />
                              </div>
                              <span className="text-gray-300">/</span>
                              <div className="flex flex-col items-center">
                                ...
                                <input
                                  ...
                                  className="w-8 bg-gray-50 border border-gray-200 text-center rounded text-xs text-indigo-700 font-black outline-none focus:bg-white"
                                />
                              </div>
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-4 shadow-xs h-fit">
            <div className="space-y-1">
              <h3 className="text-sm font-black uppercase text-slate-800 flex items-center gap-1.5">
                ...
                <span>
                  {(industry as any) === "grain"
                    ? "Place Paddy/Grain Supply Order"
                    : "Place Raw Material Supply Order"}
                </span>
              </h3>
              <p className="text-[10px] text-gray-500 leading-snug">
                {(industry as any) === "grain"
                  ? "Restock raw paddy silo volumes or processed grain by issuing orders directly to agriculture agents or farms."
                  : "Restock raw material volumes or processed products by issuing orders directly to suppliers."}
              </p>
            </div>

            <form onSubmit={handlePlaceOrder} className="space-y-3.5">
              <div className="space-y-1">
                <label className="text-[9px] uppercase font-bold text-gray-400 tracking-wider block">
                  Target Variety
                </label>
                <select
                ...
                >
                  ...
                  {grainInventory
                    .filter(
                      (g) => !commodities.some((c) => c.name === g.grainName),
                    )
                    .map((g) => (
                      <option key={g.id} value={g.grainName}>
                        {g.grainName}
                      </option>
                    ))}
                </select>
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl lg:col-span-2 space-y-4 shadow-xs">
            <div className="space-y-1">
              <h3 className="text-sm font-black uppercase text-slate-800">
                Supply Pipeline Orders Tracker
              </h3>
              <p className="text-[10px] text-gray-500 font-sans">
                Review and update custom orders. Once products arrive
                physically, click "Receive Inventory" to sync balances
                automatically.
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 text-[11px]">
                  {industryFilteredOrders.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="py-12 text-center text-gray-400 font-semibold italic"
                      >
                        No active purchase orders in pipeline. Initiate an order
                        from the supplier desk.
                      </td>
                    </tr>
                  ) : (
                    industryFilteredOrders.map((ord) => (
                      <tr
                        key={ord.id}
                        className="hover:bg-gray-50/50 transition-colors"
                      >
                        <td className="py-3 px-3 font-mono font-bold text-gray-500 uppercase">
                        ...
                        <td className="py-3 px-3 text-center">
                          <div className="font-mono text-[9.5px] text-gray-600">
                            Out: {ord.orderDate}
                          </div>
                          <div className="font-mono text-[9.5px] text-indigo-700 font-bold">
                            ETA: {ord.expectedDelivery}
                          </div>
                        </td>
                        <td className="py-3 px-3 text-center">
                          {ord.status === "Pending" ? (
                            <span className="inline-block px-2 py-0.5 bg-yellow-50 text-yellow-700 text-[9px] font-extrabold uppercase rounded-full border border-yellow-200 animate-pulse">
                              Pending
                            </span>
                          ) : (
                            <span className="inline-block px-2 py-0.5 bg-green-50 text-green-700 text-[9px] font-extrabold uppercase rounded-full border border-green-200">
                              Received
                            </span>
                          )}
                        </td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in text-gray-900">
          ...
          <div className="bg-white border border-gray-200 p-5 rounded-3xl space-y-4 shadow-xs h-fit">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-gray-500 leading-snug">
                Configure a customer's requested quote. The simulator checks
                grain silo status and computes the exact milling packaging cycle
                duration.
              </p>
            </div>
            ...
            <div className="space-y-3.5">
              ...
              <div className="space-y-1">
                <label className="text-[9px] uppercase font-bold text-gray-400 tracking-wider block">
                  Analyze Saved Quote / RFQ Folder
                </label>
                <select
                  value={testerQuoteId}
                  ...
                  {savedQuotes
                    ...
                    .map((q) => (
                      <option key={q.id} value={String(q.id)}>
                        {q.ref || `RFQ-${q.id}`} ({q.company || "Direct Buyer"}{" "}
                        - {q.items[0]?.commodity})
                      </option>
                    ))}
                </select>
              ...
              {testerQuoteId === "custom" ? (
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      {grainInventory
                        .filter(
                          (g) =>
                            !commodities.some((c) => c.name === g.grainName),
                        )
                        .map((g) => (
                          <option key={g.id} value={g.grainName}>
                            {g.grainName}
                ...
                <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-2xl text-[11px] text-indigo-900 leading-relaxed font-sans">
                  <span className="font-extrabold uppercase text-[9px] text-indigo-950 block mb-1">
                    ✔ Auto-Extracted Parameters
                  </span>
                  {(() => {
                    const q = savedQuotes.find(
                      (x) => String(x.id) === testerQuoteId,
                    );
                    ...
                    return (
                      <div className="space-y-1 font-mono text-[10.5px]">
                        <p>
                          <strong className="font-sans">Client:</strong>{" "}
                          {q?.company}
                        </p>
                        <p>
                          <strong className="font-sans">Target variety:</strong>{" "}
                          {item.commodity}
                        </p>
                        <p>
                          <strong className="font-sans">Demand Volume:</strong>{" "}
                          {(item.totalWeightKg / 1000).toFixed(2)} Tons
                        </p>
                        <p>
                          <strong className="font-sans">
                            Packaging Specification:
                          </strong>{" "}
                          {item.size} ({item.packed})
                        </p>
                      </div>
                    );
                  })()}
              ...
              <div className="p-3 bg-amber-50 rounded-2xl border border-amber-200 text-amber-900 text-[10px] space-y-1 leading-relaxed">
                <span className="font-extrabold uppercase text-[8.5px] tracking-widest text-amber-950 block flex items-center gap-1">
                  <AlertTriangle className="w-3.5 h-3.5 shrink-0" /> Milling
                  Recovery Yield Alert
                </span>
                <p>
                  {(industry as any) === "grain" ? (
                    <>
                      ...
                      from raw un-dehusked paddy.
                    </>
                  ) : (
                    "Calculated based on estimated processing yields and material recovery from raw scrap/resources."
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-6">
              <div className="flex flex-wrap items-center justify-between border-b border-gray-100 pb-3 gap-2">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 font-sans">
                    Real-time gap index check on warehouse stock lines.
                  </p>
                </div>
                ...
                {analysis.status === "ready" && (
                  <span className="px-3 py-1 bg-green-500 text-slate-950 text-[10px] font-extrabold rounded-full uppercase tracking-wider flex items-center gap-1 shadow">
                    <CheckCircle className="w-3.5 h-3.5" /> Fulfillable
                    Immediately (0 Days)
                  </span>
                )}
                {analysis.status === "warning" && (
                  <span className="px-3 py-1 bg-yellow-500 text-slate-950 text-[10px] font-extrabold rounded-full uppercase tracking-wider flex items-center gap-1 shadow animate-pulse">
                    <Clock className="w-3.5 h-3.5" /> Delay Expected (
                    {analysis.cumulativeDays} Days)
                  </span>
                )}
                {analysis.status === "critical" && (
                  <span className="px-3 py-1 bg-red-500 text-white text-[10px] font-extrabold rounded-full uppercase tracking-wider flex items-center gap-1 shadow animate-pulse">
                    <AlertTriangle className="w-3.5 h-3.5" /> Supply Wait Order
                    Needed ({analysis.cumulativeDays} Days)
                  </span>
                )}
              </div>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div className="p-3 bg-gray-50 border border-gray-100 rounded-2xl text-center">
                        ...
                        <span className="text-base font-black text-gray-900 font-sans">
                          {analysis.requiredBags?.toLocaleString()}
                        </span>
                        <span className="text-[9px] text-gray-400 block font-mono">
                          ({analysis.targetTons} T total)
                        </span>
                      </div>
                      <div className="p-3 bg-gray-50 border border-gray-100 rounded-2xl text-center">
                        <span className="text-[8px] uppercase tracking-wider text-gray-400 block font-mono">
                        ...
                        <span className="text-base font-black text-green-700 font-sans">
                          {analysis.bagsInStock?.toLocaleString()}
                        </span>
                        <span className="text-[9px] text-gray-400 block font-mono">
                          Bags stacked
                        </span>
                      ...
                      <div className="p-3 bg-gray-50 border border-gray-100 rounded-2xl text-center">
                        ...
                        <span className="text-base font-black text-rose-600 font-sans">
                          {analysis.bagsShort > 0
                            ? `-${analysis.bagsShort.toLocaleString()}`
                            : "0"}
                        </span>
                        <span className="text-[9px] text-gray-400 block font-mono">
                          Missing pieces
                      ...
                      <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-2xl text-center">
                        <span className="text-[8px] uppercase tracking-wider text-indigo-400 block font-mono">
                          Final Cycle Time
                        </span>
                        <span className="text-base font-black text-indigo-800 font-sans">
                          {analysis.cumulativeDays} Days
                        </span>
                        <span className="text-[9px] text-indigo-600 block font-mono">
                          Wait production
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50/70 border border-gray-200 rounded-2xl space-y-3.5">
                      <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider block font-sans">
                        Silo & Pile Feasibility Breakdown
                      </span>
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-[11px] font-bold">
                          <span className="text-gray-700 flex items-center gap-1">
                            ...
                            Layer 1: Stitched Ready Bags Stock Level
                          </span>
                          <span
                            className={
                              analysis.bagsShort === 0
                          ...
                          >
                            {analysis.bagsInStock} / {analysis.requiredBags}{" "}
                            Bags (
                            {analysis.bagsShort === 0
                              ? "100%"
                              : `${((analysis.bagsInStock / analysis.requiredBags) * 100).toFixed(0)}%`}
                            )
                          </span>
                        </div>
                        <div className="w-full h-2.5 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (analysis.bagsInStock / (analysis.requiredBags || 1)) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-[11px] font-bold">
                          <span className="text-gray-700 flex items-center gap-1">
                            ...
                            Layer 2: Milled Polished Rice Stack Level
                          </span>
                          <span
                            className={
                              analysis.processedRiceDeficit === 0
                                ? "text-green-700 font-black"
                                : "text-amber-700 font-extrabold"
                            }
                          >
                            {analysis.info?.processedRiceTons?.toFixed(2)} T
                            Stock vs {analysis.tonsShort?.toFixed(2)} T Shortage
                          </span>
                        </div>
                        <div className="w-full h-2.5 bg-gray-200 rounded-full overflow-hidden">
                          <div
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-[11px] font-bold">
                          <span className="text-gray-700 flex items-center gap-1">
                            ...
                            {(industry as any) === "grain"
                              ? "Layer 3: Undehusked Paddy Silo Stocks"
                              : "Layer 3: Raw Materials Stock"}
                          </span>
                          <span
                            className={
                              analysis.paddyDeficit === 0
                                ? "text-green-700 font-black"
                                : "text-rose-600 font-extrabold"
                            }
                          >
                            {analysis.info?.paddyStockTons?.toFixed(2)} T{" "}
                            {(industry as any) === "grain" ? "Paddy" : "Raw"} vs{" "}
                            {(analysis.processedRiceDeficit > 0
                              ? analysis.processedRiceDeficit /
                                (analysis.millingYield || 0.65)
                              : 0
                            ).toFixed(2)}{" "}
                            T Needed
                          </span>
                      ...
                      <div className="relative border-l-2 border-indigo-200 ml-3.5 pl-5 space-y-5">
                        {analysis.steps?.map((step: any, idx: number) => (
                          <div key={idx} className="relative">
                            ...
                            <span
                              className={`absolute -left-[29px] top-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                                step.days === 0
            ...
            <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-gray-500 leading-snug">
                  The system scans your whole quotation history to detect
                  inventory gaps so you never promise quantities you cannot
                  produce or ship.
                </p>
              </div>

              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {savedQuotes.filter((q) => q.items && q.items.length > 0)
                  .length === 0 ? (
                  <div className="p-4 text-center text-xs text-gray-400 italic font-semibold">
                    No generated quotes to evaluate in active system. Use Quote
                    Sheet tab to lock prices.
                  </div>
                ) : (
                  savedQuotes
                    .filter((q) => q.items && q.items.length > 0)
                    .map((quote) => {
                      ...
                      const invRef = grainInventory.find(
                        (g) =>
                          g.grainName.trim().toUpperCase() ===
                            item.commodity.trim().toUpperCase() ||
                          item.commodity
                            .trim()
                            .toUpperCase()
                            .includes(g.grainName.trim().toUpperCase()) ||
                          g.grainName
                            .trim()
                            .toUpperCase()
                            .includes(item.commodity.trim().toUpperCase()),
                      );

                      const weightTons = item.totalWeightKg / 1000;
                      const bSize =
                        parseFloat(item.size.replace(/[^0-9.]/g, "")) || 20;
                      const reqBags = Math.ceil(item.totalWeightKg / bSize);

                      const hasBags = invRef
                        ? invRef.readyBagsCount >= reqBags
                        : true;
                      const hasGrain = invRef
                        ? (invRef.readyBagsCount * bSize) / 1000 +
                            invRef.processedRiceTons >=
                          weightTons
                        : true;
                        ...
                        >
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-2 font-mono text-[10.5px]">
                              <strong className="text-slate-900 font-extrabold">
                                {quote.ref || `RFQ-${quote.id}`}
                              </strong>
                              <span className="text-gray-400">|</span>
                              <span className="text-gray-500 font-sans font-bold">
                                {quote.company || "Private Importer"}
                              </span>
                            </div>
                            <p className="text-[11px] text-gray-700 font-semibold font-sans">
                              {item.commodity} &middot;{" "}
                              <strong className="font-bold text-slate-800">
                                {weightTons.toFixed(1)} Tons
                              </strong>{" "}
                              ({reqBags.toLocaleString()} bags of {bSize} kg)
                            </p>
                          </div>
                          ...
                          <div className="text-right">
                            {hasBags ? (
                              ...
                              <div className="space-y-0.5">
                                <span
                                  className={`text-[9.5px] uppercase font-black block ${hasGrain ? "text-amber-700" : "text-red-700"}`}
                                >
                                  {hasGrain
                                    ? "Needs packaging stitching"
                                    : "Deficit: Requires supplier order"}
                                </span>
                                <span className="text-[9px] text-gray-400 block font-mono">
                                  Missing{" "}
                                  {invRef
